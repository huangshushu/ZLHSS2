	...
	if (mode == -1) {
		...
		if (a == -1) {
			...
			if (selection == 0) {
				...
				if(cm.getPlayer().getOneTimeLoga("透明披风2阶") > 0){
					cm.sendOk("一个账号只允许进阶1次!");
					cm.dispose();
					return;
				}
			else	if (cm.haveItem(4310081, 10) && cm.haveItem(4031997, 10) && cm.getInventory(1).getItem(1).getOwner() == "第一阶段") {
					cm.getPlayer().setOneTimeLoga("透明披风2阶");
