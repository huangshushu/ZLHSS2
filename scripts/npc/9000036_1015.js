...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			//text += "#b#v1112900##z1112900#*1（升级至四维200，攻魔100）\r\n"
			...
			if(cm.getzb() >= 2000 && cm.haveItem(1112949,1) && cm.getPlayer().getBossLog("2000赞助物品礼包",1) == 0 ){
				//cm.gainItem(1112900,-1);
				//cm.gainItem(1112900,200,200,200,200,0,0,100,100,0,0,0,0,0,0);//赞助戒指
				cm.gainItem(4310060,8);//进化石
				cm.gainItem(4310057,20);//跑商币
				cm.gainItem(4310143,30);//BOSS币
				cm.gainItem(4310027,25);//SSR时装附魔币
				cm.gainItem(4251202,20);//万能水晶
				cm.gainItem(2613000,5);//星火武器卷自选箱
				cm.gainItem(3700040,1);//二段跳
				cm.gainItem(3700048 ,1); //平定战场的人
				...
				cm.gainItem(4310009,1);//10星必成币
				cm.gainMeso(50000000);//5000W
				
				cm.getPlayer().setBossLog("2000赞助物品礼包",1,1);
