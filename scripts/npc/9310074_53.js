...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (a == -1) {
			...
			if(cm.getPlayer().getBossLog("回收3") >=15){
			...
			if (cm.canHold(3992025,688) == false) {
				...
				Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "通用版装备回收" + " : " + "恭喜" + cm.getChar().getName() + "成功回收了法弗纳装备，获得了圣诞大星*"+rand+"！大家祝贺他/她吧！"));           
				cm.dispose();
            }
        }
    }
