...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			cm.sendNext("#r您现在所选择的是#d[暴君手套邮票强化]#v1082543##v1082544##v1082545##v1082546##v1082547##k#l\r\n#d[暴君手套邮票强化]#b强化成功率:#r100%#k\r\n#b属性幅度:#r-2~+5#k\r\n#b强化消耗:#v4002001##r*1#k\r\n#d次数限制:每天上限 #r[10] #k次#l\r\n#bPS：#d由于星之力的魔化产生了不可描述的变异,所以该强化系统无论强化成功或失败,均不会扣除强化次数!\r\n");
		} else if (status == 1) {
		...
		//	var 爱心 = "#fEffect/CharacterEff/1022223/4/0#";
			...
			if (!cm.haveItem(4002001,1)&&cm.getMeso()>=500000) {
				cm.sendOk("#b对不起,邮票数量不足:#r[1]颗！#k\r\n\t   #b当前所拥有#v4002001#:#r [#c4002001#] 颗!#l");
				status = -1;
			} else if (cm.getBossLog("手套") >= 10) {
				cm.setBossLog("手套");
				...
				if (sj == 0) {
					...
					var 力量随机=Math.floor(Math.random()*5);
                    var 敏捷随机=Math.floor(Math.random()*5);
                    var 运气随机=Math.floor(Math.random()*5);
                    var 智力随机=Math.floor(Math.random()*5);
				    var 物攻随机=Math.floor(Math.random()*5);
                    var 魔攻随机=Math.floor(Math.random()*5);
					...
					Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(11,cm.getC().getChannel(),"蓝蜗牛邮票公告" + " : " + cm.getPlayer().getName() +"玩家成功 升级暴君手套 属性提升。O(∩_∩)O~",true).getBytes());
				...
				//cm.laba(cm.getPlayer().getName() + "『强化公告』" + " : " + "在拍卖-强化成功！大家一起恭喜他（她）吧！", 9);
					...
					Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(11,cm.getC().getChannel(),"蓝蜗牛邮票失败公告" + " : " + cm.getPlayer().getName() +"玩家失败 升级暴君手套 属性减少。O(∩_∩)O~",true).getBytes());
