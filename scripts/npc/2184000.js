...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == startmap) {
        if (status == 0) {
            ...
            rwpz += "#d       当前已挑战希拉[#g"+cm.getBossLog('希拉') + "#b/#r3#k]次#n#k\r\n";
            rwpz += "\r\n#e#b推荐人數：" + minPlayers + " - " + maxPlayers + "#n#e    推荐等级：" + minLevel[selection] + " - " + maxLevel[selection] + "#n#k";
            rwpz += "\r\n#e#d当前已進行：#e#g" +cm.getBossLog('希拉')  + " #n#k次\r\n";
            ...
            } else if (cm.getBossLog('希拉') >= 3) {//判断BOSS每日次数
           ...
           // } 
            ...
            //} 
                 ...
                 if (cm.getPlayerCount(262030300) <= 0 && cm.getPlayerCount(262030300) <= 0 && cm.getPlayerCount(262030300) <= 0 && cm.getPlayerCount(262030300) <= 0 && cm.getPlayerCount(262030300) <= 0) {
                            cm.sendOk("即将传送");
                            cm.warpParty(262030300);
                            cm.getMap(262030300).resetFully();
                            cm.spawnMob(8870000, 164, 196);
                            cm.getPlayer().setBossLog('希拉');
                         // cm.喇叭(2,"『终极BOSS』：【" + cm.getChar().getName() + "】带领他的队伍悍不畏死的去希拉了");
                    } else {
