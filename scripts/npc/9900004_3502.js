...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
		    ...
		    if(cm.haveItem(4310088,48)){
				//cm.getPlayer().setOneTimeLog("宠物攻击卷轴");
				cm.gainItem(2048088, 1);//宠物攻击卷轴
				cm.gainItem(4310088, -48);
		    cm.sendOk("购买成功！");
			cm.全服黄色喇叭("[赞助商店] : 恭喜玩家 "+cm.getPlayer().getName()+" 成功购买了 宠物攻击卷轴。")
            cm.dispose();
			}else{
            cm.sendOk("#r你的赞助币不足");
            cm.dispose();
