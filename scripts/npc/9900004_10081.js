	...
	if (mode == -1) {
		...
		if (a == -1) {
			...
			if (selection == 0) {
				...
				if(cm.getPlayer().getOneTimeLoga("透明" + cm.getInventory(1).getItem(1).getItemId() + "2阶") > 0){
				...
				cm.dispose();
				} else if (cm.haveItem(4310081, 10) && cm.haveItem(4031997, 30) && cm.getPlayer().getCSPoints(1) >= 10000 && cm.getPlayer().getCSPoints(2) >= 5000 && cm.getPlayer().getMeso() >= 20000000 && cm.getInventory(1).getItem(1).getOwner() == "第一阶段") {
					cm.getPlayer().setOneTimeLoga("透明" + cm.getInventory(1).getItem(1).getItemId() + "2阶");
					...
					cm.sendOk("强化成功，当前装备等级为 “#r第二阶段#k”。\r\n #r#e全属性增加[2]点 攻击力增加[1]点 魔法力增加[1]点");
					
					//cm.全服道具公告("【透明强化】", "玩家 ["+cm.getName()+"]成功将透明装备强化为第二阶段！恭喜恭喜！", item);
					cm.dispose();
				} else {
