...
function action(mode, type, selection) {
    if (mode == -1) {
		...
		var 物理破甲 = cm.getPlayer().获取角色潜能汇总值(100)+cm.getPlayer().获取附魔汇总值(12);
		var 魔法破甲 = cm.getPlayer().获取角色潜能汇总值(101)+cm.getPlayer().获取附魔汇总值(11);
		...
		var 魔法防御2 = 0;
        if (status == 0) {
            cm.sendGetText("    Hi~#b#h ##k，这里是#b" + cm.开服名称() + "#k破甲计算系统，请输入目标防御值，我会根据你的属性来计算你可以造成多少输出。#k\r\n\r\n请输入防御值；");
        } else if (status == 1) {
			fee = cm.getText();
            物理防御 = fee;
			魔法防御 = fee;
            if (fee > 0 && fee < 100000) {
				...
				var 文本 = "     Hi~#b#h ##k，输入目标防御后，会显示出你对目标造成的伤害占比，提升破甲值可以提升输出比，破甲会优先计算百分比破甲，再计算固定破甲值。\r\n\r\n";
                文本 += "     #d当前的状态 : "
				if (冒险岛勇士()== 1) {
					百分比 += 冒险岛勇士等级()* 3 / 10;
					文本 += "#s1121000#"
				...
				文本 += "\r\n"
                文本 += "     #d物理破甲值 : #r" + 物理破甲 + "#k\r\n"
                文本 += "     #d物理输出率 : #d" + (100 - cm.防御计算(物理防御2 - 物理破甲)).toFixed(2) + "%#k\r\n"
                文本 += "     #d魔法破甲值 : #b" + 魔法破甲 + "#k\r\n"
                文本 += "     #d魔法输出率 : #d" + (100 - cm.防御计算(魔法防御2 - 魔法破甲)).toFixed(2) + "%#k\r\n"
                cm.说明文字(文本);
                cm.dispose();
            } else {
                cm.dispose();
...
function  冒险岛勇士() {
    if (cm.getPlayer().isActiveBuffedValue(1121000) ||
