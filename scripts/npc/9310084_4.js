	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			cm.dispose();
		       ...
		       if (!cm.haveItem(4251302, 2) || cm.getMeso() <= 40000000)  {
		            cm.sendOk("#v4251302#或金币不足4000万!");
				    cm.dispose();
          } else if (cm.getBossLog("qianghua4") == 1){//每天一次
...
var sj = Math.floor(Math.random()*2);
		 cm.gainItem(4251302,-2);
		 cm.gainMeso(-40000000);//扣除多少金币
