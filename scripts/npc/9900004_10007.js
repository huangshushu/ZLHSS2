...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		// cm.getPlayer().dropMessage("chrid " + chrid);//调试
		if(chrid > 0){
			
			var name = MapleCharacter.getCharacterNameById(chrid);
			
			if(离开师门(chrid)){
				cm.sendOk("您已将 " + name + " 逐出师门。");
				cm.全服黄色喇叭("【师徒系统】 : 青山不改绿水长流, "+cm.getPlayer().getName()+"  将 ["+name+"] 逐出了师门。");
				// cm.全服黄色喇叭("[师徒系统] : 青山不改绿水长流, " + cm.getPlayer().getName() + " 将 " + name + " 逐出了师门。");
			}else{
				cm.sendOk("因不明原因，无法逐出师门。");
			}
