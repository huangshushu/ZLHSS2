...
function action(mode, type, selection) {
    ...
    switch (select) {
        ...
        case 1:
        {
		if (cm.getBossLog("每日签到") >= 1 || cm.getBossLog("奖励一条龙") >= 1  ) {
            cm.sendOk("签到失败原因：\r\n1.您今天已经签到过了\r\n2.你的副本一条龙还没有完成\r\n");
			cm.dispose();	
        }else{
            cm.setBossLog("每日签到");
		...
		case 2:
        {
		if (cm.getBossLog("每日签到") <= 6  || cm.getBossLog("每日签到7") >= 1    ) {
            cm.sendOk("签到失败原因：\r\n1.您已经领取过了\r\n2.您的签到次数不足7次\r\n");
			cm.dispose();	
        }else{
		...
		case 3:
        {
		if (cm.getBossLog("每日签到") <= 14 || cm.getBossLog("每日签到15") >= 1   ) {
            cm.sendOk("签到失败原因：\r\n1.您已经领取过了\r\n2.您的签到次数不足15次\r\n");
			cm.dispose();	
        }else{
            cm.setBossLog("每日签到15");
		...
		case 4:
        {
		if (cm.getBossLog("每日签到") <= 29 || cm.getBossLog("每日签到30") >= 1   ) {
            cm.sendOk("签到失败原因：\r\n1.您已经领取过了\r\n2.您的签到次数不足30次\r\n");
			cm.dispose();	
        }else{
