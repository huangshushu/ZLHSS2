...
function action(mode, type, selection) {
    if (mode == -1) {
	    ...
	    var a2 = "#L2#" + 正方箭头 + "#b重置#r#v1032186##z1032186# 属性50 攻魔50\r\n\r\n需要1000万冒险币制作\r\n";
	    
            cm.sendSimple("这里是阿尔泰耳环重置哟，请选择：\r\n"+a2+"");
        } else if (status == 1) {
		...
		if (cm.haveItem(1032186,1)&&cm.getMeso()>=10000000) {
			...
			Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "阿尔泰耳环重置" + " : " + "恭喜『" + cm.getChar().getName() + "玩家成功重置恢复了阿尔泰耳环。O(∩_∩)O哈哈~"));
           
			cm.sendOk("#z1032186#已经重置好了，去试一下吧");
			cm.dispose();
			return;
		} else {
			cm.sendOk("你的材料不足!!!");
