            ...
            if (xx == 0) {
                if (jilv == 0) {} else if (jilv != 0) {
                    if (cm.getMeso() < cost) {
                        ...
                        jiaru = GetRandomNum(0, jilv+1);
                        if (jiaru == 0) {
                            costa = cost * jilv
                            cm.gainMeso(costa)
                            cm.gainItem(4001228,1);//精准之心
                            cm.sendOk("#b恭喜,您已经大获全胜...");
							//cm.laba("[博彩公告]" + cm.getPlayer().getName() + " : " + "在博彩中赢得" + costa + "金币。",2);
                            cm.worldMessage("[博彩公告]" + cm.getPlayer().getName() + " : " + "在博彩中赢得" + costa + "金币。");
                            status = -1;
                        } else {
                            cm.gainMeso(-cost)
                            cm.worldMessage("[博彩公告]" + cm.getPlayer().getName() + " : " + "在博彩中输了" + cost + "金币。");
                            cm.sendOk("#b悲剧啊.你输了....");
                            status = -1;
                        }
                    }
