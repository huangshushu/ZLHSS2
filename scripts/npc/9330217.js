...
function start() {
    if (cm.getMapId() == startmap) {
        ...
        cm.sendSimple("#e<Boss - " + PQLog[0] + ">#n\r\n\r\n#b#h0# \n\#k你现在想挑战这个BOSS副本吗?#r入场需要#v4033235#请获取后入场\r\n" + text);
    } else {
    ...
    if (cm.getMapId() == startmap) {
        if (status == 0) {
            ...
            if (cm.getParty() == null) { //判断组队
                ...
                cm.sendNext("等级 " + minLevel[chs] + " 以上 " + maxLevel[chs] + " 以下才可以入场。");
		    } else if (!cm.isAllPartyMembersHaveItem(4033235, 1)) {
                cm.sendNext("#r#e \"" + cm.getNotHaveItemMemberName(2430865, 1) + "\" #k#n没有#i4033235##t4033235#,请确认!");
            } else if (!cm.isAllPartyMembersAllowedPQ(PQLog[chs], maxenter[chs])) {
                cm.sendNext("#r#e \"" + cm.getNotAllowedPQMemberName(PQLog[chs], maxenter[chs]) + "\" #k#n次数已经达到上限了。");
