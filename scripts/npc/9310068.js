...
var 概率 = Math.floor(Math.random()*65);
var 强化次数 = 1;
var 枫叶 = 5570000;
var 枫叶数量 = 1;
var 力量 = Math.floor(Math.random()*4);
var 敏捷 = Math.floor(Math.random()*4);
var 运气 = Math.floor(Math.random()*4);
var 智力 = Math.floor(Math.random()*4);
var 攻击 = Math.floor(Math.random()*4);
var 魔攻 = Math.floor(Math.random()*4);
...
function action(mode, type, selection) {
	...
	//	citem.setLevel(citem.getLevel()+1);//加升级次数
		...
		cm.playerMessage(5, "发出一道闪光，成功强化装备可升级次数!");	 
		cm.全服公告(cm.getPlayer().getName() + "『无限金锤』" + " : " + "使用金锤子成功强化装备可升级次数！");
