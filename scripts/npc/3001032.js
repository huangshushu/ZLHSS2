...

{
			...
			//  text += "#e#r" + 星星 + "这里是扎昆的祭台" + 星星 + ".\r\n"//3     
			    text += "#e#r" + 彩虹 +"#e<BOSS - 麦格纳斯>#n\r\n进图要求:#v4110001#x1\r\n#r(玩家每天可以入场#e3次#n,入场记录#e每天午夜0点#n将会初始化)\r\n#b\r\n"
    ...
    if (cm.getParty() == null) 
		 ...
		 else if (cm.getBossLog("麦格纳斯") >= 3)//召唤20次
   {
    cm.sendOk("你好,系统限定每天只能挑战3次!");
    cm.dispose();
   }
   else if(cm.getMonsterCount(401060300) > 0)
   {
		m.sendOk("请先消灭掉该地图已经召唤出的BOSS!");
      cm.dispose();
   }
               //5
...
Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(11, cm.getClient().getChannel(), "『系统公告』" + " : " + "[" + cm.getChar().getName() + "]开始挑战麦格纳斯，大家一起为他(她)鼓掌!"));			cm.dispose();
                        ...
                        cm.sendOk("检测：你要有#v4031250#才能进入地图!");
                        cm.dispose();
	 }
      }
