...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "\t\t\t\t#e#b  " + cm.开服名称() + "1000元赞助礼包 #k	#n\r\n"
            text += "#b\t#v1072153# 全属双攻 +5#k  #l\r\n"//3
		  ...
		  if(cm.读取累计赞助()>999 && cm.getPlayer().getOneTimeLoga("1000元累计赞助礼包") == 0){
				cm.给属性装备(1072153, 0, 0, 5, 5, 5, 5, 0, 0, 5, 5, 0, 0, 0, 0, 0, 0);//V2勋章
				cm.gainItem(2340000, 10);//祝福
				cm.gainItem(2049100,5);
				
		    cm.sendOk("领取成功！");
			cm.全服黄色喇叭("[累计充值] : 恭喜玩家 "+cm.getPlayer().getName()+" 成功领取了1000元累充礼包。")
			cm.getPlayer().setOneTimeLoga('1000元累计赞助礼包');
            cm.dispose();
			}else{
