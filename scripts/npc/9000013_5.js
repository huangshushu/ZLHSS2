...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			cm.sendNext("#r您现在所选择的是#d[品克缤武器卷轴强化]#v1382199##k#l\r\n#b强化成功率:#r100%#k\r\n#b#b强化消耗:#v2043819##r*1#k\r\n#d次数限制:每天上限 #r[32] #k次#l\r\n");
		} else if (status == 1) {
		...
		//	var 爱心 = "#fEffect/CharacterEff/1022223/4/0#";
			...
			if (!cm.haveItem(2043819,1) || !cm.haveItem(1382199,1) || cm.getMeso()<2000000) {
				cm.sendOk("#b对不起,游戏币不足200万，或品克缤武器卷不足:#r[1]张！#k\r\n\t   #b当前所拥有#v2043819#:#r [#c2043819#] 张!#l");
				status = -1;
				...
				if (sj == 0) {
					...
					var 力量随机=Math.floor(Math.random()*0);
                    var 敏捷随机=Math.floor(Math.random()*0);
                    var 运气随机=Math.floor(Math.random()*0);
                    var 智力随机=Math.floor(Math.random()*0+5);
				    var 物攻随机=Math.floor(Math.random()*0);
                    var 魔攻随机=Math.floor(Math.random()*0+7);
