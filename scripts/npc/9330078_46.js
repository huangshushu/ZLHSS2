...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            text += "#e#d制作#v1422140#\r\n需要#v1422156# #v4000270#x500#v4000245#x8#v4000244#x8#v4000175#x3#v4001126#x1666，冒险币#r3333W#d\r\n搜集好道具我就可以为您制作了.#l\r\n\r\n"//3
            text += "#L1##r确定制作#l\r\n"//3
            cm.sendSimple(text);
        } else if (selection == 1) {
			//1
            ...
            cm.dispose();
			}else */if(cm.haveItem(1422156,1)&& cm.haveItem(4000270,500)&& cm.haveItem(4000245,8)&& cm.haveItem(4000244,8)&& cm.haveItem(4000175,3)&& cm.haveItem(4001126,1666)&& cm.getMeso()>=33333333 ){
				...
				cm.getC().getSession().write(MaplePacketCreator.addInventorySlot(type, toDrop));
          			cm.sendOk("制作成功！");
Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "『装备制作』" + " : " + "[" + cm.getChar().getName() + "]成功升级获得了法弗纳闪电锤"));
           		        cm.dispose();
			}else{
           		 cm.sendOk("您的材料不足！");
            		cm.dispose();
