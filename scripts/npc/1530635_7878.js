...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var zyms = "";
        zyms = "#fMob/1210102.img/move/0##fMob/1210102.img/move/0##b伴侣 - MS#fMob/1210102.img/move/0##fMob/1210102.img/move/0#\r\n";
        zyms += "当前在线信息: #r" + cm.getOnlineTime() + "#k 点。\r\n\r\n";
        zyms += "                 #L0##b查看规则#l \r\n\r\n";
        ...
        if (selection == 0) {
            cm.sendOk("#fMob/1210102.img/move/0##fMob/1210102.img/move/0##b伴侣 - MS#fMob/1210102.img/move/0##fMob/1210102.img/move/0#\r\n规则如下：\r\n#b1#k、每日0时到24时，只要您在线，就可以获得在线时间点数，每1分钟可获得1点在线点数。\r\n#b2#k、可以用在线时间点数兑换道具，各种极品道具等您来拿！\r\n#b3#k、若您当日24时内不兑换在线时间点数，则第二天0点在线时间点数会自动清零，请您注意。\r\n#b4#k、若不小心掉线，您可以重新登陆，时间会累计。\r\n#b5#k、各种奖励是随机的，也可能随时增加或者取消某些道具，在线时间点数越大，兑换的道具会越好！\r\n#b6#k、在线6小时以上，都有超极品道具送。而且如果您在线时间超过了12小时，那么将获得一份丰富的大礼。如果在线能到23小时，那么最好的道具将会给您。#k\r\n#b7#k、在线点数为#b 301#k到#b 700#k点可以兑换获得#r1#k个#v4031504##k\r\n#b8#k、在线点数为#b700#k到#b1380#k点可以兑换获得#r1#k个#v4031505#\r\n#b9#k、在线点数为#b1380#k到#b1440#k点可以兑换获得#r1#k个#v4031506#");
            cm.dispose();
        } else if (selection == 1) {
            cm.sendYesNo("您目前的在线时间点数为#b " + cm.getOnlineTime() + " #k点，请问您是否要兑换奖励？");
        } else if (selection == 2) {
            status = 2;
            cm.sendSimple("#fMob/1210102.img/move/0##fMob/1210102.img/move/0##b伴侣 - MS#fMob/1210102.img/move/0##fMob/1210102.img/move/0#\r\n#k礼盒数量:#v4031504#:#r" + cm.itemQuantity(4031504) + " #k个  #v4031505#: #r" + cm.itemQuantity(4031505) + " #k个 #v4031506#: #r" + cm.itemQuantity(4031506) + " #k个\r\n#L0##b使用#v4031504#兑换物品#k#l \r\n#L1##b使用#v4031505#兑换物品#k#l \r\n#L2##b使用#v4031506#兑换物品#l");
        }
    } else if (status == 2) {
        if (cm.getPQLog("签到奖品A") >= 1) {
            cm.sendOk("抱歉 一天只可以领一次");
            cm.dispose();
            return;
        }
        if (cm.getPQLog("签到奖品B") >= 1) {
            cm.sendOk("抱歉 一天只可以领一次");
            cm.dispose();
            return;
        }
        if (cm.getPQLog("签到奖品C") >= 1) {
            cm.sendOk("抱歉 一天只可以领一次");
            cm.dispose();
            return;
        }
        if (cm.getPQLog("签到奖品D") >= 1) {
            cm.sendOk("抱歉 一天只可以领一次");
            cm.dispose();
            return;
        }
        if (cm.getPQLog("签到奖品E") >= 1) {
            cm.sendOk("抱歉 一天只可以领一次");
            cm.dispose();
            return;
        }
