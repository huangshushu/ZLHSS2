                ...
                if (jilv == 0) {} else if (jilv != 0) {
                    if (cm.getMeso() < cost) {
                        ...
                        if (jiaru == 0) {
                            costa = cost * jilv
                            cm.gainMeso(costa)
                  Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "『赌博系统』" + " : " + "[" + cm.getChar().getName() + "]恭喜大获全胜 大家快去打劫他~"));          cm.sendOk("#b恭喜,您已经大获全胜...");
						//	cm.laba("[博彩公告]" + cm.getPlayer().getName() + " : " + "在博彩中赢得" + costa + "金币。",2);
                            status = -1;
                        } else {
                            cm.gainMeso(-cost)
                                  Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "『赌博系统』" + " : " + "[" + cm.getChar().getName() + "]输得只剩下裤衩 珍爱生命 远离赌博~"));           cm.sendOk("#b悲剧啊.你输了....");
                            status = -1;
                        }
                    }
                }
