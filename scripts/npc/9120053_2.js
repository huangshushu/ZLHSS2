...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == startmap) {
        if (status == 0) {
            ...
            rwpz += "#d       当前已挑战生化魔女欧碧拉[#g"+cm.getPlayer().getBossLog('生化魔女欧碧拉') + "#b/#r3#k]次#n#k\r\n";
            rwpz += "\r\n#e#b推荐人數：" + minPlayers + " - " + maxPlayers + "#n#e    推荐等级：" + minLevel[selection] + " - " + maxLevel[selection] + "#n#k";
            rwpz += "\r\n#e#d当前已進行：#e#g" +cm.getPlayer().getBossLog('生化魔女欧碧拉')  + " #n#k次\r\n";
            ...
            } else if (cm.getPlayer().getLevel() < 150) {//判断玩家角色等级
                cm.sendNext("组队成员等级 " + 150 + " 以上 " + 255 + " 以下才可以入场。");
            } else if (cm.getBossLog('生化魔女欧碧拉') > 3) {//判断BOSS每日次数
           ...
           // } 
            ...
            //} 
                  ...
                  if (cm.getPlayerCount(802000825) <= 0 && cm.getPlayerCount(802000825) <= 0 && cm.getPlayerCount(802000825) <= 0 && cm.getPlayerCount(802000825) <= 0 && cm.getPlayerCount(802000825) <= 0) {
                            cm.sendOk("即将传送");
                            cm.warpParty(802000825);
                            cm.getMap(802000825).resetFully();
                            cm.spawnMob(8220012, 383, 335);
                            cm.getPlayer().setBossLog('生化魔女欧碧拉');
                         cm.喇叭(2,"『终极BOSS』：【" + cm.getChar().getName() + "】带领他的队伍悍不畏死的去生化魔女欧碧拉了");
                    } else {
