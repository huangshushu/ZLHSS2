...
{
	var nextmap = cm.getC().getChannelServer().getMapFactory().getMap(350051250);
	if (mode == -1)
	{
		cm.dispose();
	}
	...
	if (status == 0)
	{	
			if (cm.getMonsterCount(350051250) > 0){
			cm.sendOk("请击碎排风口的透明屏障..");
			cm.dispose();
			}else{
...
if(cm.getMonsterCount(350051250) <= 999){
		

	
}			
			
                           cm.warpParty(350060000,"sp");
