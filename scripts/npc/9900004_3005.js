...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "\t\t\t\t#e#b  " + cm.开服名称() + "5000元赞助礼包 #k	#n\r\n"
            text += "#b\t#v1142803# 全属双攻 +15  #l\r\n"//3
		  ...
		  if (cm.getInventory(1).isFull(0)) {
			...
			//4
			//5
		    if(cm.读取累计赞助()>4999 && cm.getPlayer().getOneTimeLoga("5000元累计赞助礼包") == 0){
				cm.给属性装备(1142803, 0, 0, 15, 15, 15, 15, 0, 0, 15, 15, 0, 0, 0, 0, 0, 0);//V2勋章
				cm.gainItem(2340000, 50);//祝福
				cm.gainItem(2049100,50);
				
		cm.sendOk("领取成功！");
			cm.全服黄色喇叭("[充值] : 恭喜玩家 "+cm.getPlayer().getName()+" 成功领取了5000元累充礼包。")
			cm.getPlayer().setOneTimeLoga('5000元累计赞助礼包');
            cm.dispose();
			}else{
