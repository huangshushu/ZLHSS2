    ...
    if (status == 0) {
        if (cm.getPlayerStat("LVL") >= 50) {
            if (cm.getQuestStatus(100200) != 2 && cm.getQuestStatus(100200) != 1) {
                ...
                cm.sendOk("你想被允许挑战残暴炎魔大怪物的任务吗？  嗯……。我#b阿杜比斯#k觉得你有能力承担这项任务。但是再这之前，你必须完成我交给你的任务。  但是你要小心点。");
                cm.dispose();
                return;
            ...
            if (cm.haveItem(4000082, 30)) { // take away items, give eyes of fire, complete quest
                if (cm.canHold(4001017)) {
                    if (cm.canHold()) {
                        ...
                        cm.sendNext("冶炼好了。 看到左边的门了吗？它就是通往残暴炎魔祭台的门。 不过你需要 #b#t4001017##k 才能进入里面。让我看看有多少人能进入到那个恐怖的地方？");
        ...
        if (selection == 0) { //ZPQ
            ...
            if (cm.haveItem(4031061) && !cm.haveItem(4031062)) {
                // good to go
                cm.sendYesNo("你已经成功通过了第一阶段。你还有很长的路才能到达残暴炎魔的祭台。所以，你想好挑战下一个阶段了吗？");
            } else {
            ...
            if (teethmode == 2 && cm.haveItem(4031061) && cm.haveItem(4031062)) {
                ...
                cm.sendYesNo("好吧， 你已经完成了早期的阶段。  现在， 努力一点我可以帮你得到进入残暴炎魔祭台所需要的 火焰的眼。 但是， 我的牙齿最近有点疼。  你见过一个牙医在冒险岛世界的故事吗？  哦，我听说僵尸们有几颗金牙。我需要你找到 #b30 个僵尸丢失的金齿#k 。这样我就可以自己制造一些假牙。然后我可以帮你拿到你想要的物品\r\n任务要求：\r\n#i4000082##b x 30 个");
            } else {
                cm.sendNext("请完成上一阶段的任务再来挑战此阶段。");
                cm.dispose();
            }
            ...
            if (cm.getQuestStatus(100200) == 2) {
                ...
                cm.sendYesNo("你想收买我？哈哈，可以啊！但你必须给我 #e300,000,000#n 枫币，我就可以让你直接跳过任务。");
                status = 3;
            }
        }
    } else if (status == 2) {
        ...
        if (cm.getPlayer().getMeso() < 300000000) {
            cm.sendNext("你好像没有足够的枫币来支付，请检查一下再来。");
