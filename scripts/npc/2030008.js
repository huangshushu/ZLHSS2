  ...
  if (status == 0) {
    if (cm.getPlayerStat("LVL") >= 50) {
      if (cm.getQuestStatus(100200) != 2 && cm.getQuestStatus(100200) != 1) {
        ...
        cm.sendOk(
          "你想被允许挑战扎昆大怪物的任务吗？  嗯……。我#b阿杜比斯#k觉得你有能力承担这项任务。但是再这之前，你必须完成我交给你的任务。  但是你要小心点。"
        );
        cm.dispose();
        return;
      ...
      cm.sendOk(
        "按照你目前的情况，你还不能满足进行这项任务的能力，当你变的強大的时候，再來找我吧！"
      );
      cm.dispose();
    }
  } else if (status == 1) {
    if (teethmode == 1) {
      if (cm.haveItem(4000082, 30)) {
        if (cm.canHold(4001017)) {
          ...
          cm.sendNext(
            "冶炼好了。 看到的门了吗？它就是通往扎昆祭台的门。 不过你需要 #b#t4001017##k 才能进入里面。让我看看有多少人能进入到那个恐怖的地方？"
          );
    ...
    if (selection == 0) {
      ...
      if (cm.haveItem(4031061) && !cm.haveItem(4031062)) {
        cm.sendYesNo(
          "你已经成功通过了第一阶段。你还有很长的路才能到达扎昆的祭台。所以，你想好挑战下一个阶段了吗？"
        );
      } else {
      ...
      if (teethmode == 2 && cm.haveItem(4031061) && cm.haveItem(4031062)) {
        cm.sendYesNo(
          "如果你想得到更多的#b火焰的眼#k， 你需要给我 #b30 个僵尸丟失的金延迟#k。 你有更多的金牙要给我吗？"
        );
      ...
      if (cm.getQuestStatus(100200) == 2) {
        ...
        cm.sendYesNo(
          "你想收买我？哈哈，可以啊！但你必须给我 #e3,000,000#n 金币，我就可以让你直接跳过任务。"
        );
        status = 3;
      }
    }
    ...
    if (stage == 1) {
      ...
      if (teethmode == 2) {
        if (
          ...
          cm.sendNext(
            "我不认为你帶來了30个 僵尸丟失的金牙呢……。请快点找來，我就会给你需要的东西。"
          );
          cm.dispose();
        }
      } else {
