...
function getLuckeyMoney(which) {
    ...
    var ps = cm.sql_Select("SELECT * FROM LuckMoneyData where id =" + monArr[which] + " and  used < AllowPeople")
    //TODO 再次判断是否已经领取
    var ii = cm.getItemInfo();
    cm.getPlayer().dropMessage(-1, monArr[which]);
    for (key in ps) {
        switch (parseInt(ps[key].get("luckmoneytype"))) {
            case 0://金币
                cm.gainMeso(parseInt(ps[key].get("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(ps[key].get("LuckyMoney")) + " 金币红包。");
                break;
            case 1://点券
                cm.gainNX(1, parseInt(ps[key].get("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(ps[key].get("LuckyMoney")) + " 点卷红包。");
                break;
            case 2://抵用
                cm.gainNX(2, parseInt(ps[key].get("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(ps[key].get("LuckyMoney")) + " 抵用卷红包。");
                break;
            case 3://现金
                cm.addHyPay(-parseInt(ps[key].get("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(ps[key].get("LuckyMoney")) + " 现金红包。");
                break;
            case 4://道具
                cm.gainItem(parseInt(ps[key].get("LuckyMoney")), 1);
                cm.sendOk("领取成功啦！！！");
                cm.worldSpouseMessage(0x24, "『拼手速红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + ii.getName(parseInt(ps[key].get("LuckyMoney"))) + " 道具红包。");
                break;
        }
        iUsed(monArr[which], parseInt(ps[key].get("used")) + 1);
        cm.setBossLog("管理员红包" + monArr[which]);
    }
}

function iUsed(uid, times) {
    return cm.sql_Update("update LuckMoneyData set used = ? where id =" + uid);
}
...
function setLuckeyMoney(monType, monNumber, monMax) {
    return cm.sql_Update("INSERT INTO LuckMoneyData(id, luckmoneytype, LuckyMoney, AllowPeople, used) value(?,?,?,?,?)", null, monType, monNumber, monMax, 0);
}

function delMoney(uid) {
    return cm.sql_Update("delete from LuckMoneyData where id = " + uid + "");
}