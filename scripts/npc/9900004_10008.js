...
function action(mode, type, selection) {
	...
	if (status == 0) {
	    cm.sendYesNo("想清楚了么？你确认要叛离师门么？");
		// cm.getPlayer().dropMessage("开师门\r\n");
		
	} else if (status == 1) {
		
		if(离开师门(cm.getPlayer().getId())){
			cm.sendOk("已成功离开师门\r\n");
			// cm.全服黄色喇叭("[师徒系统]: 青山不改绿水长流, " + cm.getPlayer().getName() + " 叛离了师门。");
			cm.全服黄色喇叭("【师徒系统】 : 青山不改绿水长流, "+cm.getPlayer().getName()+"  叛离了自己的师门。");
		}else{
			cm.sendOk("你并没有师门啊！\r\n");
		}
		cm.dispose();
