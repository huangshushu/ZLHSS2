...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (cm.haveItem(2613001,1) == false) {
				cm.sendOk("#b抱歉，你拥有的#v2613001##r#z2613001##b数量不足！");
				status = -1;
                ...
                var 魔攻随机=Math.floor(Math.random()*3+9);
				...
				cm.sendOk("#r恭喜你，强化成功，为武器提升了四维"+四维随机+"，攻击"+魔攻随机+"！");
				Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(11,cm.getC().getChannel(),"星火武器强化" + " : 恭喜" + cm.getPlayer().getName() +"使用星火单手武器魔法力卷成功为武器提升了四维"+四维随机+"，攻击"+魔攻随机+"，大家祝贺他/她！",true).getBytes());
				status = -1;
			}
		}
	}
