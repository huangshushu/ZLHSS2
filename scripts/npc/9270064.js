 ...
 */
 ...
 function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        var text = "#r找我可以把背包里所有抽奖获得的时装分解每件15000点券！\r\n#e#d" + 小烟花 + "回收导致损失不予补偿！" + 小烟花 + "#n#k\r\n\r\n";
		text += "可回收以下装备\r\n#v1082505##v1072782##v1052579##v1702399##l\r\n"
		...
		if(selection == 1){
			if(cm.判断背包其他栏().isFull()){
				...
				for (var i = 0; i < 卷轴列表.length; i++) {
					...
					if(mount > 0){
						...
						cm.gainNX(15000*mount);
						是否有卷轴 = true;
					}
				}
				if(是否有卷轴){
					cm.sendOk("回收完毕，#b共计回收了 #r" + 兑换数量 + " #b个装备，获得 #r" + 15000*兑换数量 + "点券\r\n");
										cm.getPlayer().setOneTimeLoga("回收2");
										cm.worldMessage(6, "[抽奖回收] : 恭喜 " + cm.getPlayer().getName() + " 回收时装获得" + 15000*兑换数量 + "点券 " );
				}else{
			...
			cm.sendOk("不想回收吗？…找我可以把背包里所有抽奖获得的时装分解每件15000点券！\r\n\r\n");
			cm.dispose();
