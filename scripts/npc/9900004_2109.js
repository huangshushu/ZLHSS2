...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "\t#e#r  成功领取了100元累充技能礼包 #k	#n\r\n\r\n"
            text += "#b\t#s4111006##d 二段跳  (领取后换线生效键码3)#l\r\n"//3
		   ...
		   if (cm.getInventory(1).isFull() || cm.getInventory(5).isFull() || cm.getInventory(4).isFull()) {
		     ...
		     cm.dispose();
				...
				cm.sendOk("二段跳#s4111006#领取成功，#b请换【频道】后查看【键码3】");
			//	cm.gainMeso(2000000);
			//	cm.gainNX(5000);
				
		cm.sendOk("领取成功！");
			cm.全服黄色喇叭("[累计充值] : 恭喜玩家 "+cm.getPlayer().getName()+" 成功领取了100元累充技能礼包。")
