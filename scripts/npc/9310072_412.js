...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (cm.haveItem(2046914,1) == false) {
				cm.sendOk("#b抱歉，你拥有的#v2046914##r#z2046914##b数量不足！");
				status = -1;
				...
				cm.sendOk("#r恭喜你，强化成功，为武器提升了四维5，魔力7！");
				Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(11,cm.getC().getChannel(),"宿命武器强化" + " : 恭喜" + cm.getPlayer().getName() +"使用宿命正义单手武器魔力卷成功为武器提升了四维5，魔力7，大家祝贺他/她！",true).getBytes());
				status = -1;
			}
		}
	}
