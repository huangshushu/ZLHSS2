...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendNext("嗨，我是 #b南哈特#k 我可以帮助你们这些新兵蛋子快速转职哦~~！");
        } else if (status == 1) {
            ...
            if(cm.getJob() >= 2000 || cm.getJob() < 1000){
                cm.sendOk("其他职业不能找我转职");
				cm.dispose();
                return;
            }
            ...
            if (cm.getJob() == 410) {
                ...
                cm.serverNotice("[转职系统]: 恭喜 [" + cm.getPlayer().getName() + "] 在 南哈特 快速转职成功！");
            } else if (cm.getJob() == 110) {
                cm.serverNotice("[转职系统]: 恭喜 [" + cm.getPlayer().getName() + "] 在 南哈特 快速转职成功！");
            } else if (cm.getJob() == 120) {
                cm.serverNotice("[转职系统]: 恭喜 [" + cm.getPlayer().getName() + "] 在 南哈特 快速转职成功！");
    ...
    cm.teachSkill(5221010,0,5);
            ...
            if(cm.getJob() == 1000 && cm.getLevel()>=10){
                ...
                cm.serverNotice("[转职系统]: 恭喜 [" + cm.getPlayer().getName() + "] 在 南哈特 快速转职为 " + cm.职业(cm.判断职业()) + "！");
                cm.sendOk("转职成功！加油锻炼，当你变的强大的时候记的来找我哦！");
            } else if(parseInt(cm.getJob() / 100) >10 && cm.getLevel()>=30 && cm.getJob()%100 == 0){
            ...
            if(cm.getJob() == 2000 && cm.getLevel() >=10){
                ...
                cm.serverNotice("[转职系统]: 恭喜 [" + cm.getPlayer().getName() + "] 在 南哈特 快速转职为战神职业！");
                cm.sendOk("转职成功！加油锻炼，当你变的强大的时候记的来找我哦！");
            } else if(cm.getJob() == 2100 && cm.getLevel() >=30){
                cm.changeJob(2110);
