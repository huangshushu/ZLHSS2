...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		//	var 爱心 = "#fEffect/CharacterEff/1022223/4/0#";
			...
			if (!cm.haveItem(3992015,1)) {
				cm.sendOk("#b对不起,您强化材料数量不足:#r[1]颗！#k\r\n\t   #b当前所拥有#v3992015#:#r [#c3992015#] 颗!#l");
				status = -1;
			} else if (cm.getBossLog("klgw3") >= 20) {
				//cm.setBossLog("klgw1");
				...
				if (sj == 0) {
					...
					var 力量随机=Math.floor(Math.random()*4+3);
                    var 敏捷随机=Math.floor(Math.random()*4+3);
                    var 运气随机=Math.floor(Math.random()*4+3);
                    var 智力随机=Math.floor(Math.random()*4+3);
				    var 物攻随机=Math.floor(Math.random()*4+3);
                    var 魔攻随机=Math.floor(Math.random()*4+3);
