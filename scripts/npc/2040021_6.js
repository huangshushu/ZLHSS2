...
function action(mode, type, selection) {
    if (mode == -1) {
	    ...
	    var a2 = "#L2#" + 正方箭头 + "#b重置#r#v1073057##z1073057# 属性50 攻魔50\r\n\r\n需要1000万冒险币制作\r\n";
	    
            cm.sendSimple("这里是寻宝鞋子重置哟，请选择：\r\n"+a2+"");
        } else if (status == 1) {
		...
		if (cm.haveItem(1073057,1)&&cm.getMeso()>=10000000) {
			
			cm.gainItem(1073057, -1);
			...
			Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "寻宝鞋子重置" + " : " + "恭喜『" + cm.getChar().getName() + "玩家成功重置恢复了寻宝鞋子。O(∩_∩)O哈哈~"));
           
			cm.sendOk("#z1073057#已经重置好了，去试一下吧");
			cm.dispose();
			return;
		} else {
			cm.sendOk("你的材料不足!!!");
