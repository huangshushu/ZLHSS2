...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == 180000001) {
        ...
        CareId = selection;
        var result = cm.sql_Select("SELECT r.accountid, r.characterid, r.carebosname, r.value, r.time, c.name, r.LimitBreak FROM (SELECT * FROM carebosringk ORDER BY value ASC) r, characters AS c WHERE r.characterid = c.id AND r.carebosname = '" + CareBo[CareId] + "' GROUP BY r.characterid ORDER BY r.value ASC, r.time ASC LIMIT 9;")
