...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "#e#b我是伟大的BOSS召唤师“和尚”，能召唤各系BOSS\r\n请选择你要挑战的BOSS\r\n"
		...
		if (cm.getBossLog("algs") >= 50) {
		cm.sendOk("一天只能打50次艾里葛斯");
		return false;
	}
		...
		if (cm.getBossLog("pkb") >= 3) {
		cm.sendOk("一天只能打3次品克缤");
		return false;
	}
			if(cm.getLevel() >= 140 && cm.haveItem(4000463,5)&& cm.getMap().getAllMonstersThreadsafe().size() == 0){ //物品条件
            cm.gainItem(4000463,-5);
			cm.setBossLog("pkb");
			cm.spawnMobOnMap(8820001,1,547, 94,910000021,2100000000);
