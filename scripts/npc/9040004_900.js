...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		    var text = "\t\t\t#e欢迎来到#b回顾冒险岛#r师徒系统#k!#n\r\n";
            for (i = 0; i < 10; i++) {
                text += "";
            }
			text += "你需要建立师门吗？\r\n"
...
var jrsm = victim.getOneTimeLog(TDid);
			if (victim.getLevel() >= 50){
				...
				cm.dispose();
				return;
			} else if (cm.getOneTimeLog("收徒") - cm.getOneTimeLog("出师") - tcsm >= 3){
				cm.sendOk("你的师门已经收过3个徒弟了");
				cm.dispose();
				return;
			} else {
					cm.setOneTimeLog("收徒");
			...
			if (cm.isLeader()) {
				...
				cm.setOneTimeLog(-TDid);
				//cm.setmoneyb(200);
				//cm.gainItem(2040807,3);//10低贝
				cm.gainItem(4001003,1);//10贝勒
				cm.gainItem(4251202,3);//圣诞大星
				cm.gainItem(2049100,20);//必成币
				cm.gainNX(+10000);//电卷
				cm.gainItem(2340000,10);//祝福
				victim.modifyCSPoints(2,10000);//队友获得
				cm.sendOk("你带徒"+victim.getName()+"出师成功!\r\n徒弟获得：1W抵用卷\r\n师傅获得：#v4001003#*1、#v4251202#*3、#v2049100#*20、#v2340000#*10。");
			...
			cm.sendOk("师门系统介绍：\r\n徒弟等级：必须小于50级\r\n师父等于：必须大于80级\r\n\r\n徒弟达到120级即可出师\r\n\r\n徒弟出师获得 #r20000#k抵用卷\r\n师父出师获得：#r5000点券、#v2040807#*3、#v4001003#*1、#v4251202#*3、#v2049100#*20、#v2340000#*30。\r\n\r\n\r\n");
			cm.dispose();
		} else if (selection == 7){
			var id = cm.getPlayer().getId();
			if (cm.getPlayer().getLevel() > 149){
				...
				cm.dispose();
				return;
			} else if (cm.getPlayer().getBossLog("收徒") >= 1 && cm.getOneTimeLog(id) == 0){
				cm.setOneTimeLog(id);
				cm.sendOk("师徒关系确认成功！");
