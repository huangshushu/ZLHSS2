        ...
        if (status == 0) {
			if(cm.getPlayer().getMapId() == 271040100){
				...
				//  text += "#e#r" + 星星 + "这里是扎昆的祭台" + 星星 + ".\r\n"//3     
					text += "#e#r" + 彩虹 +"#e<BOSS - 贝伦>#n\r\n进图要求:#v4110001#x1\r\n#r(玩家每个账号每天可以入场#e" + 挑战次数 + "次#n,入场记录#e每天午夜0点#n将会初始化)\r\n#b\r\n"
				  text += "#L1##d" + 小烟花 + "申请进入贝伦(队员会同时移动。)#l\r\n" 
				  text += "#L3##d" + 小烟花 + "申请进入进阶贝伦(队员会同时移动。)#l\r\n"				  
			...
			if (selection == 3) {
				if (cm.getParty() == null) { // 没有组队
					cm.sendOk("请组队后和我谈话。");
					cm.dispose();
				} else if (cm.getPlayerCount(105200800) > 1 || cm.getPlayerCount(105200810) > 1) {
					cm.sendOk("已经有人挑战进阶贝伦了，你无法进入!");
					cm.dispose();
				} else if (!cm.isLeader()) { // Not Party Leader
				   cm.sendOk("请让你的队长和我说话~");
				   cm.dispose();
				} else if (cm.判断团队每日a("进阶贝伦", 进阶挑战次数) == 0) {//召唤20次
					...
					cm.重置目标地图(105200800); 
					cm.重置目标地图(105200810); 					
					cm.warpParty(105200800);
					cm.给团队每日a("进阶贝伦");
					// cm.spawnMobOnMap(8920100, 1, 40, 125, 105200310);
					cm.给当前地图时钟(10, true, true);
					cm.全服喇叭(6, "[系统公告]：" + cm.getPlayer().getName() + " 带领队伍开始挑战进阶贝伦，大家一起为他（她）鼓掌！");
					cm.dispose();
				...
				if (cm.getParty() == null) { // 没有组队
					cm.sendOk("请组队后和我谈话。");
					cm.dispose();
				} else if (cm.getPlayerCount(105200400) > 1 || cm.getPlayerCount(105200410) > 1) {
					cm.sendOk("已经有人挑战贝伦了，你无法进入!");
					cm.dispose();
				} else if (!cm.isLeader()) { // Not Party Leader
				   cm.sendOk("请让你的队长和我说话~");
				   cm.dispose();
				} else if (cm.判断团队每日a("贝伦", 挑战次数) == 0) {//召唤20次
					...
					cm.重置目标地图(105200400); 
					cm.重置目标地图(105200410); 					
					cm.warpParty(105200400);
					cm.给团队每日a("贝伦");
					// cm.spawnMobOnMap(8920100, 1, 40, 125, 105200310);
					cm.给当前地图时钟(10, true, true);
					cm.全服喇叭(6, "[系统公告]：" + cm.getPlayer().getName() + " 带领队伍开始挑战贝伦，大家一起为他（她）鼓掌！");
					cm.dispose();
				...
				if(cm.getPlayer().getMapId() == 105200410){
					cm.warp(105200000);
					cm.dispose();
				}
			}
