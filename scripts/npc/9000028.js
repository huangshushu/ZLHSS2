...
var 攻击 = Math.floor(Math.random()+18);
var 魔攻 = Math.floor(Math.random()+18);
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("欢迎来到#r" + cm.getChannelServer().getServerName() + "#b强化中心\r\n\r\n#k#r每次消耗#v2046996#x1 增加第一格装备18点攻击");	
		} else if (status == 1) {	
		   if(cm.getPlayer().getBossLog("qianghua") >= 强化次数){//判断bosslog记录
...
//		citem.setLuk(citem.getLuk()+运气);//给运气
		...
		cm.全服公告(cm.getPlayer().getName() + "『强化中心』" + " : " + "使用惊人卷轴成功强化装备！");
