...
var selection;
var itemlist = Array(
1302297,1312173,1322223,1332247,1382231,
1402220,1412152,1422158,1432187,1442242,
1452226,1462213,1472235,1482189,1492199);
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (a == -1) {
			...
			if(cm.getPlayer().getBossLog("回收4") >=15){
				cm.sendOk("今日回收已经超过15次，无法继续回收了！");
				cm.dispose();
				return;
			}
			var rand = Math.floor(Math.random()*500+388);
			if (cm.haveItem(itemlist[jg], 1) == false) {
                cm.sendOk("#b很遗憾，你的背包内并没有这个装备，请确认！");
                cm.dispose();
				return;
			}
			if (cm.canHold(3992025,888) == false) {
				...
				Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "通用版装备回收" + " : " + "恭喜" + cm.getChar().getName() + "成功回收了漩涡装备，获得了圣诞大星*"+rand+"！大家祝贺他/她吧！"));           
				cm.dispose();
            }
        }
    }
