...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selection == 10000) {
            ...
            cm.sendSimple("活动礼包:\r\n "
                    + "#rA.紫色苹果*5  转蛋券*5  混沌卷轴60%*1 猎人的幸运*1 400枫点。#k\r\n "
                    + "#rB.紫色苹果*10 转蛋券*10 混沌卷轴60%*3 猎人的幸运*3 800枫点。#k\r\n "
                    + "#rC.紫色苹果*15 转蛋券*15 混沌卷轴60%*5 猎人的幸运*5 1200枫点。#k\r\n "
                    + "#r#z1902024# 独角兽骑宠(7天)。#k\r\n "
                    + "#r#z1912017# 独角兽驯服水晶(7天)。#k\r\n "
        ...
        if (selection == 100) {
            if (cm.getPlayer().getLevel() >= 60) {
                if (cm.getPlayer().getTGJF() >= 1) {
                    if (cm.canHold() && cm.canHoldByType(2, 7) && cm.canHoldByType(5, 2)) {
                        cm.getTGLB(1);
                        cm.sendOk("领取成功。");
                        cm.dispose();
                        return;
        ...
        if (selection == 102) {
            if (cm.getPlayer().getLevel() >= 60) {
                if (cm.getPlayer().getTGJF() >= 3) {
                    if (cm.canHold() && cm.canHoldByType(1, 3) && cm.canHoldByType(2, 7) && cm.canHoldByType(5, 2)) {
                        cm.getTGLB(3);
                        cm.sendOk("领取成功。");
                        cm.dispose();
                        return;
