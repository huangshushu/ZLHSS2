	...
	if (mode == -1) {
		...
		if (a == -1) {
			...
			if (selection == 0) {
				...
				if(cm.getPlayer().getOneTimeLoga("透明" + cm.getInventory(1).getItem(1).getItemId() + "传说") > 0){
				...
				if(dmID!=1012057 && dmID!=1022048 && dmID!=1032024 && dmID!=1702224 && dmID!=1072153 && dmID!=1082102) {
					...
					cm.dispose();
				} else if (cm.haveItem(4310129, 30) && cm.haveItem(4031997, 60) && cm.getPlayer().getCSPoints(1) >= 25000 && cm.getPlayer().getCSPoints(2) >= 5000 && cm.getPlayer().getMeso() >= 50000000 && cm.getInventory(1).getItem(1).getOwner() == "史诗") {
					cm.getPlayer().setOneTimeLoga("透明" + cm.getInventory(1).getItem(1).getItemId() + "传说");
					...
					//cm.全服道具公告("【透明强化】", "玩家 ["+cm.getName()+"]成功将透明装备强化为传说！恭喜恭喜！", item);
					cm.dispose();
				} else {
