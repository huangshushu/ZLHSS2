...
{
	var nextmap = cm.getC().getChannelServer().getMapFactory().getMap(863010330);
	if (mode == -1)
	{
		cm.dispose();
	}
	...
	if (status == 0)
	{	
			if (cm.getMonsterCount(863010330) > 0){
			cm.sendOk("请击败贝勒德的右手臂..");
			cm.dispose();
			}else{
...
if(cm.getMonsterCount(863010330) <= 999){
		

	
}			
			
                           cm.warpParty(863010210,"out00");
