...
function start() {
    if (cm.getBossLog("挑战副本3") > 1) {
        ...
        cm.sendOk("每天挑战2次 需要190级~");
        cm.dispose();
} else if (cm.getPlayerCount(108010301) > 0 && cm.getBossLog('挑战暴力熊3') < 2|| cm.getPlayerCount(108010301) > 0) {
...
function action(mode, type, selection) {
    var em = cm.getEventManager("JJpkb");
    if (em == null) {
        ...
        if (cm.getMeso() > 25000000) {
            ...
            cm.setBossLog("挑战副本3");
			Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "进阶BOSS" + " : " + "恭喜『" + cm.getChar().getName() + "』带领他的队伍挑战三头犬，伟大的勇士，祝您好运！"));
