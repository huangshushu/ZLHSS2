    ...
    if (prop == null || prop.equals("0")) {
        ...
        if (squadAvailability == -1) {
            ...
            if (props != null && props.equals("true")) {
                ...
                cm.sendNext("很抱歉你的远征队队长离开了现场，所以你不能再返回战场。");
                cm.safeDispose();
            }
        }
    } else {
        ...
        if (props != null && props.equals("true")) {
            ...
            var say = "\r\n" + (eim == null ? "eim is null" : eim.getProperty("isSquadPlayerID_" + cm.getPlayer().getId())) + "\r\n";
            if ((eim != null) && (eim.getProperty("isSquadPlayerID_" + cm.getPlayer().getId()) != null)) {
                status = 13;
                say += "#b现在是否要重新返回远征队所在场地？";
                say += "\r\n#r#L1#重新返回远征队所在场地#l";
                cm.sendSimple(say);
            } else {
                eim = cm.getDisconnected("HorntailBattle");
                if (eim == null) {
                    cm.sendNext("其它远征队，正在对战中。" + say);
                    cm.safeDispose();
                } else {
                    cm.sendNext("其它远征队，正在对战中。" + say);
                    cm.safeDispose();
