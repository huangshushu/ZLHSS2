...
//【配置区】-----------------------------------------------------
var cf_itemId = 2290133;//消耗指定道具学习技能


var cf_skills = [
	//技能ID，技能描述，技能购买消耗，技能初始等级，技能最高等级，限定VIP等级，是否允许升级
	[21120005,"提升1级进阶巨熊咆哮。",1,31,30,0,true],
...
function action(mode, type, selection) {
    ...
    if(status == 0){
		...
		say += "#d\t战神，如果你有#v"+cf_itemId+"#的话，属性修仙3次可以学习\r\n\r\n";
		say += "\t\t\t\t #L1##b学习技能#l\r\n";
		say += "\t\t\t\t #L2##b升级技能#l\r\n";
        cm.sendSimple(say);
    }else if(status == 1){
		...
		if(cm.haveItem(cf_itemId, thisSel[2])){
			cm.sendYesNo("系统将把#s" + thisSel[0] + "#放在 技能栏 键");
		} else {
		...
		chr.setBossLog("study:"+thisSel[0],1,1);
		cm.teachSkill(thisSel[0], thisLevel+31,thisSel[4]);
		//chr.changeKeybinding(30,1,thisSel[0]);
