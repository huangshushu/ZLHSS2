...
function start() {
 	if (cm.getBossLog("挑战副本43") > 2){
		...
		cm.dispose();
            ...
            cm.dispose();
	} else if (cm.getPlayerCount(108010101) > 0 && cm.getBossLog('挑战暴力熊43') < 3|| cm.getPlayerCount(108010101) > 0) {
...
function action(mode, type, selection) {
    var em = cm.getEventManager("beiyong5");
    if (em == null) {
        ...
        if (cm.getMeso() > 20000000) {
            em.startInstance(cm.getParty(),cm.getMap());
				...
				cm.setBossLog("挑战副本43");
				Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "进阶BOSS" + " : " + "恭喜『" + cm.getChar().getName() + "』带领他的队伍挑战威尔，伟大的勇士，祝您好运！"));
