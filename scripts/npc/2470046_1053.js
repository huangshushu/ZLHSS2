...
function action(mode, type, selection) {
    ...
    if (status == 0) {
      ...
      //  var selStr = "";
        ...
        sel = selection ;
		if (sel == 1) {
			cm.sendYesNo("是否要花费20万点卷购买一个独特烟花皮肤？");
		} else if (sel == 3) {
			cm.sendYesNo("是否要花费20万点卷购买一个独特棉花皮肤？");
		} else if (sel == 2) {
			cm.sendYesNo("是否要还原成默认皮肤？还原不收取费用，并且还原之后无法撤销。");
		} else {
			cm.sendYesNo("是否要花费10万点卷购买一个#i"+sel+":##t"+sel+"#？");
		}
        //cm.dispose();
    } else if (status == 2) {
		if (sel <=0)
