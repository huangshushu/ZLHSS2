...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		text += "\t\t#r#v4001126#x100000\t #b点券#rx100000\t #b金币#rx300000000\r\n"
		text += "#k#v1114404#成品属性: #b#v1114404# #v1113084# #v1112952# #v1112951#属性之和\r\n";
		text += "\t\t\t\t\t#L1##d[合成]\r\n";
		cm.sendSimple(text);
	} else if (status == 1) {
		if(selection == 1){

			if(cm.判断金币() < 300000000){
				cm.sendOk("你的金币不够啊！");
				cm.dispose();
				return;
			}else if(!cm.haveItem(4001126, 100000)){
				cm.sendOk("你的#v4001126#不够啊！");
				cm.dispose();
				return;
			}else if(cm.判断点券() < 100000){
				cm.sendOk("你的点券不够啊！");
				cm.dispose();
				return;
			}
			...
			if(rings1.size() > 0 && rings2.size() > 0 && rings3.size() > 0 ){
				...
				if(ring1 != null && ring2 != null && ring3 != null ){
					...
					cm.全服道具公告("[装备制作]", "恭喜 " + cm.getPlayer().getName() + " 制作了黑暗之力戒指~", ring_s);
					cm.sendOk("恭喜你合成了#v1114404#");
					cm.dispose();
					return;
				}else{
					cm.sendOk("你的戒指不满足条件啊！");
					cm.dispose();
					return;
				}
			}else{
