 ...
 */
 ...
 function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        var text = "#r找我可以把背包里所有抽奖获得的时装分解每件5000点券！\r\n#e#d" + 小烟花 + "回收导致损失不予补偿！" + 小烟花 + "#n#k\r\n\r\n";
		text += "可回收以下装备\r\n#v1070031##v1112745##v1004042##v1003544##v1102653##v1004044##v1003545##v1112734##v1113021##v1102555##v1102659##l\r\n\r\n"
		...
		if(selection == 1){
			if(cm.判断背包其他栏().isFull()){
				...
				for (var i = 0; i < 卷轴列表.length; i++) {
					...
					if(mount > 0){
						...
						cm.gainNX(5000*mount);
						是否有卷轴 = true;
					}
				}
				if(是否有卷轴){
					cm.sendOk("回收完毕，#b共计回收了 #r" + 兑换数量 + " #b个装备，获得 #r" + 5000*兑换数量 + "点券\r\n");
										cm.getPlayer().setOneTimeLoga("回收1");
										cm.worldMessage(6, "[抽奖回收] : 恭喜 " + cm.getPlayer().getName() + " 回收时装获得" + 5000*兑换数量 + "点券 " );
				}else{
			...
			cm.sendOk("不想回收吗？…找我可以把背包里所有抽奖获得的时装分解每件5000点券！\r\n\r\n");
			cm.dispose();
