...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (cm.haveItem(2046173,1) == false) {
				cm.sendOk("#b抱歉，你拥有的#v2046173##r#z2046173##b数量不足！");
				status = -1;
				...
				cm.sendOk("#r恭喜你，强化成功，为武器提升了四维5，攻击7！");
				Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(11,cm.getC().getChannel(),"宿命武器强化" + " : 恭喜" + cm.getPlayer().getName() +"使用宿命正义双手武器攻击卷成功为武器提升了四维5，攻击7，大家祝贺他/她！",true).getBytes());
				status = -1;
			}
		}
	}
