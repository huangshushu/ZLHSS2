 ...
 **/
 ...
 function getPrize(eim, cm) {
     ...
     cm.setBossLog('每日废弃副本次数');
     var a = cm.getBossLog("每日废弃副本次数");
     if (cm.getPlayer().getLevel() <= 120){//废弃带副本奖励
         cm.gainItem(itemSet[sel], qty);
         if (a <= 1) {
             cm.gainItem(itemSet[sel], qty);
         }
         cm.playerMessage(5, "恭喜你完成废弃副本！");
     } else {
         //cm.setBossRankCount("越级副本出席", 2);
         cm.givePartyLevelItems(1, 34, 1, 4001229, 1);//敏捷之心
         if (a <= 1) {
            cm.gainItem(itemSet[sel], qty);
         }
         cm.setBossRankCount("越级废弃副本总次数");
         cm.playerMessage(5, "恭喜你带新人完成废弃副本！");
     }
     cm.setBossRankCount("废弃副本总次数");
     //废弃副本通关记2点
     cm.playerMessage(5, "恭喜你获得废弃副本点数！");
     cm.setBossRankPoints("废弃副本总点数", 1);
     cm.setBossRankCount("所有副本总次数");
     cm.removeAll(4001007);
     cm.removeAll(4001008);
     //cm.gainItem(1072369, 1);//绿粘液鞋子
