    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.getMeso() < fee) {
                ...
                cm.sendOk("对不起,你的金币不足100万,所以不能参于此次下注!");
                cm.dispose();
            } else if (cm.getText() > 10000000) {
                cm.sendOk("哦不好意思哦!这里最大赌注不能超过1000万!");
                cm.dispose();
...
cm.serverNotice("『赌场公告』：哇.可怜的"+ cm.getChar().getName() +"，在中赌场输的只剩下内裤，珍爱生命 远离赌博!"); 
	                    cm.dispose(); 
	                } 
	                else if (chance == 2) { 
	                    cm.gainMeso(-fee); 
                    ...
                    cm.serverNotice("『赌场公告』：恭喜"+ cm.getChar().getName() +"，在中赌场赢得3倍的金币，大家一起为他祝贺吧!");
                    cm.dispose();
                }
            }
        }
