...
function action(mode, type, selection) {
    if (mode == -1) {
		...
		if(status == 0){
		    ...
		    if(cm.haveItem(4310088, 30 * selection)){
				//cm.getPlayer().setOneTimeLog("10W破功石");
				cm.gainItem(4310025, 1 * selection);//角色栏增加券
				cm.gainItem(4310088, - 30 * selection);
		    cm.sendOk("购买成功！");
			cm.全服黄色喇叭("[赞助商店] : 恭喜玩家 "+cm.getPlayer().getName()+" 成功购买了 岛主银币x" + selection + "。")
            cm.dispose();
			}else{
            cm.sendOk("#r你的赞助币不足");
            cm.dispose();
