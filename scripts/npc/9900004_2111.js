...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "\t#e#r  300累计充值技能 #k	#n\r\n\r\n"
            text += "#b\t#s2311003##d 神圣祈祷  (领取后换线生效键码4)#l\r\n"//3
		   ...
		   if (cm.getInventory(1).isFull() || cm.getInventory(5).isFull() || cm.getInventory(4).isFull()) {
		     ...
		     cm.dispose();
				...
				cm.sendOk("神圣祈祷#s2311003#领取成功，#b请换【频道】后查看【键码4】");
			//	cm.gainMeso(2000000);
			//	cm.gainNX(5000);
				
		cm.sendOk("领取成功！");
			cm.全服黄色喇叭("[累计充值] : 恭喜玩家 "+cm.getPlayer().getName()+" 成功领取了300元累充技能礼包。")
