...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		if (cm.getPlayer().getMapId() == 105050400) {
			if (cm.getPlayer().getLevel() < 200 && cm.haveItem(4032496)) {
			...
			if (cm.getParty() == null) {
				...
				while (it.hasNext()) {
					var cPlayer = it.next();
					if ((cPlayer.getLevel() >= 10 && cPlayer.getLevel() < 200) || cPlayer.getJobId() == 900) {
						levelValid += 1;
					} else {
						next = false;
					}
				...
				if (!next) {
					cm.sendOk("在同一张地图上，您需要你的组队人数为1人.");
					cm.dispose();
					return;
				}
			}
			...
			if (cm.getPlayer().getLevel() < 200 && cm.haveItem(4032496)) {
				if (cm.getParty() == null) {
					...
					if (next) {
						...
						cm.sendOk("在同一张地图上，在同一张地图上，您需要你的组队人数为1人.");
					}
				}
			} else {
				cm.warp(105050400,0);
