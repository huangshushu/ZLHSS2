...
function action(mode, type, selection) {
    if (mode == 1) {
      ...
      if (status == 0) {
        ...
        msg += "\r\n#k任意在我这里制作的#b60级#k武器#b" + 武器扣除数量 + "件#k！";
		msg += "\r\n";
        msg += "#g----------------------------------------------\r\n";
		...
		if(!haveWeapenToRequire(weaponToRequire, 武器扣除数量)){
			cm.sendNext("你身上没有足够的在我这里制作的#r60级武器#k。");
			cm.dispose();
			return;
		}
		...
		Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(3, cm.getClient().getChannel(), "『合成中心』" + " : " + "[" + cm.getChar().getName() + "]成功合成了80级武器！！")); 
