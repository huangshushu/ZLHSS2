    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            if (cm.getMeso() < fee) {
                ...
                cm.sendOk("对不起,你的金币不足1000万,所以不能参于此次下注!");
                cm.dispose();
            } else if (cm.getText() > 100000000) {
                cm.sendOk("哦不好意思哦!这里最大赌注不能超过1E!");
                cm.dispose();
                ...
                cm.dispose();
                 ...
                 if (chance <= 1) { 
	                    ...
	                    cm.sendNext("哦··你的运气不怎么好哦··哈哈 再来一把嘛!"); 
                            cm.serverNotice("『赌场公告』：哇.可怜的"+ cm.getChar().getName() +"，在大赌场输的只剩下内裤，珍爱生命 远离赌博!"); 
	                    cm.dispose(); 
	                } 
	                else if (chance == 2) { 
	                    cm.gainMeso(-fee); 
	                    ...
	                    cm.sendNext("哦··你的运气不怎么好哦··扣除金币\r\n骗你的啦 赠送你5个黄金枫叶!"); 
                            cm.serverNotice("『赌场公告』：哇.可怜的"+ cm.getChar().getName() +"，在大赌场赢得五个黄金枫叶，珍爱生命 远离赌博!"); 
	                    cm.dispose(); 
	                } 
                else if (chance >= 6) {
                    cm.gainMeso(fee * 3);
                    ...
                    cm.serverNotice("『赌场公告』：恭喜"+ cm.getChar().getName() +"，在大赌场赢得3倍的金币，大家一起为他祝贺吧!");
                    cm.dispose();
                }
            }
        }
