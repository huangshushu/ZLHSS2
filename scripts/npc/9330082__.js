    ...
    if (status == 0) {
        ...
        if (sel == 102) {
            if (cm.haveItem(3993002)) {
                ...
                dhPoints += intPoints + 9000;
                record.setCustomData("" + dhPoints + "");
                cm.sendOk("完成! 请查看你的点数。");
                cm.dispose();
                return;
            } else {
                cm.sendOk("请检查你是否有#i3993002#x1。");
                cm.dispose();
                return;
            }
        }
        ...
        if (select >= 3) {
            cm.sendYesNo("请问你确定是否要兑换#v" + items[select - 3][sel][0] + "##t" + items[select - 3][sel][0] + "#\r\n\r\n需要#r" + items[select - 3][sel][1] + "#kBS点数");
        } else {
            cm.sendYesNo("请问你确定是否要兑换点数?");
            cm.dispose();
        }
    } else if (status == 3) {
...
function gainReward(intPoints, record, index) {
    if (intPoints >= items[index][sel][1]) {
        if (cm.canHoldByType(3, items[index][sel][2])) {

        }
        if (items[index][sel][2] == 10) {
            if (cm.canHoldByType(3, items[index][sel][2])) {
