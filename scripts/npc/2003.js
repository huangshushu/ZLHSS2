...
function action(mode, type, selection) {
  ...
  if (status == 0) {
    ...
    if (sel == 0)
      cm.sendNext(
        "好，我来教你如何移动。 使用 #方向左键#k 就能在平台上移动了，按下 #bAlt#k 可以进行跳跃。 有些鞋子能提升你的速度以及跳跃力。"
      );
    else if (sel == 1)
      cm.sendNext(
        "好，击退怪物很简单，每个怪物有自己的血条，你可以使用武器将他们杀死。当然，如果怪物等级越高，你越难击退它们。"
      );
    else if (sel == 2)
      cm.sendNext(
        "接下来告诉你如何剪取物品，当你击退怪物时，会有机会掉落宝物以及枫币，当地上有物品时，按下#bZ#k 或是 数字键盘上的 #b0 来捡取物品。"
      );
    else if (sel == 3)
      cm.sendNext(
        "你好奇地找出当你死会发生什么吗？ 当你的HP归零时，你会变成幽灵。 而地上会出现一块墓碑，而你无法移动，但是你还是可以聊天。"
      );
    else if (sel == 4)
      cm.sendNext(
        "什么时候你可以选择你的职业？哈哈哈，别紧张，我的朋友啊～每个职业都有等级的限制。通常在8等和10等之间会进行。"
      );
    else if (sel == 5)
      cm.sendNext(
        "你想要知道这个岛屿吗？ 这里是枫之岛，这座岛屿浮在天空上。由于浮在天空上，强大的怪物们无法靠近。这里非常和平，非常适合新手。"
      );
    else if (sel == 6)
      cm.sendNext(
        "你想成为#b战士#k？ 摁...那我建议你到维多利亚港，寻找一个叫做#r勇士之村#k的战士村庄以及去找寻#bDances with Balrog#k。 他会教你如何成为一个战士。 喔对了，有件很重要的事，你必须达到等级10才能成为战士！"
      );
    else if (sel == 7)
      cm.sendNext(
