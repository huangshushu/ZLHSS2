...
function action(mode, type, selection) {
	...
	if (cm.getChar().getMapId() != 500000000 && cm.getChar().getMapId() != 702000000 && cm.getChar().getMapId() != 800000000 && cm.getChar().getMapId() != 600000000 && cm.getChar().getMapId() != 540000000 && cm.getChar().getMapId() != 550000000 && cm.getChar().getMapId() != 541000000) {
		if (status == 0) {
			status = 1;
			if (cm.getJob() == 0) {
				cm.sendNext("世界这么多!不多出去走走怎么行呢?                            #b回顾冒险岛旅游团#k只需要#b100000金币#k。");
			} else {
				cm.sendNext("世界这么多!不多出去走走怎么行呢?                            #b回顾冒险岛旅游团#k只需要#b100000金币#k");
			}
		} else if (status == 1) {
			cm.sendSimple("现在就可以去往 #b泰国的水上市场#k游览一番。在各旅游地我都会为大家提供满意热诚的服务。那么请准备好，新手可以9折优惠。\r\n#b#L0#查看旅游线路.#k#l");
		} else if (status == 2) {
var selStr = "现在就可以去往 #b泰国的水上市场#k游览一番。在各旅游地我都会为大家提供满意热诚的服务。那么请准备好，新手可以9折优惠。#b";
				if (cm.getJob() == 0) {
			...
			cm.sendYesNo("你已经决定好，确定要去 #b#m" + maps[selectedMap][0] + "##k 吗？那么你将要付给我 #b" + cost +"金币#k. 你真的想去？");
		} else if (status == 4) {
			if (cm.getMeso() < cost) {
