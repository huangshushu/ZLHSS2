...
function start() {
    if (cm.getBossLog("挑战副本1") > 1) {
        ...
        cm.sendOk("每天挑战2次 需要190级~");
        cm.dispose();
} else if (cm.getPlayerCount(209000001) > 0 && cm.getBossLog('挑战暴力熊1') < 2|| cm.getPlayerCount(209000001) > 0) {
...
function action(mode, type, selection) {
    ...
    if (em == null) {
        ...
        if (cm.getMeso() > 25000000) {
            ...
            cm.setBossLog("挑战副本1");
				Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(12, cm.getClient().getChannel(), "进阶BOSS" + " : " + "恭喜『" + cm.getChar().getName() + "』带领他的队伍挑战阿卡，伟大的勇士，祝您好运！"));
