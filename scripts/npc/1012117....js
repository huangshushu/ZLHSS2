...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        txt = "\t\t\t\t　　　#r#e" + Icon[82][1] + " 造型社 " + Icon[82][1] + "#k#n\r\n\r\n";
...
function getHair() {
    ...
    for (var i in Care_Sql) {
        Hair.push(parseInt(Care_Sql[i].get("hair")))
    }
    return Hair;
}
...
function getHair_Plus(name_id, ver) {
    var Hair = [];
    var Hair_plus = [];
    var ps = cm.sql_Select("select concat('发型ID - ',hair,'　发型名称 - ',hairname) as care, hair from carehair where concat('发型ID - ',hair,'　　　发型名称 - ',hairname) LIKE '%?%'", name_id);
    for (var i in ps) {
        Hair.push(parseInt(ps[i].get("hair")))
        Hair_plus.push(ps[i].get("care"))
    }
    if (ver == true) {
        return Hair;
    } else if (ver == false) {
