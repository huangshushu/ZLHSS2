        ...
        if (status == 0) {
			if(cm.getPlayer().getMapId() == 271040100){
				...
				//  text += "#e#r" + 星星 + "这里是扎昆的祭台" + 星星 + ".\r\n"//3     
					text += "#e#r" + 彩虹 +"#e<BOSS - 皮埃尔>#n\r\n进图要求:#v4110001#x1\r\n#r(玩家每个账号每天可以入场#e" + 挑战次数 + "次#n,入场记录#e每天午夜0点#n将会初始化)\r\n#b\r\n"
				  text += "#L1##d" + 小烟花 + "申请进入皮埃尔(队员会同时移动。)#l\r\n"  
				  text += "#L3##d" + 小烟花 + "申请进入进阶皮埃尔(队员会同时移动。)#l\r\n"
			...
			if (selection == 3) {
				if (cm.getParty() == null) { // 没有组队
					cm.sendOk("请组队后和我谈话。");
					cm.dispose();
				} else if (cm.getPlayerCount(105200600) > 1 || cm.getPlayerCount(105200610) > 1) {
					cm.sendOk("已经有人挑战进阶皮埃尔了，你无法进入!");
					cm.dispose();
				} else if (!cm.isLeader()) { // Not Party Leader
				   cm.sendOk("请让你的队长和我说话~");
				   cm.dispose();
				} else if (cm.判断团队每日a("进阶皮埃尔", 进阶挑战次数) == 0) {//召唤20次
					...
					cm.重置目标地图(105200600); 
					cm.重置目标地图(105200610); 					
					cm.warpParty(105200600);
					cm.给团队每日a("进阶皮埃尔");
					// cm.spawnMobOnMap(8920100, 1, 40, 125, 105200310);
					cm.给当前地图时钟(10, true, true);
					cm.全服喇叭(6, "[系统公告]：" + cm.getPlayer().getName() + " 带领队伍开始挑战进阶皮埃尔，大家一起为他（她）鼓掌！");
					cm.dispose();
				...
				if (cm.getParty() == null) { // 没有组队
					cm.sendOk("请组队后和我谈话。");
					cm.dispose();
				} else if (cm.getPlayerCount(105200200) > 1 || cm.getPlayerCount(105200210) > 1) {
					cm.sendOk("已经有人挑战皮埃尔了，你无法进入!");
					cm.dispose();
				} else if (!cm.isLeader()) { // Not Party Leader
				   cm.sendOk("请让你的队长和我说话~");
				   cm.dispose();
				} else if (cm.判断团队每日a("皮埃尔", 挑战次数) == 0) {//召唤20次
					...
					cm.重置目标地图(105200200); 
					cm.重置目标地图(105200210); 					
					cm.warpParty(105200200);
					cm.给团队每日a("皮埃尔");
					// cm.spawnMobOnMap(8920100, 1, 40, 125, 105200310);
					cm.给当前地图时钟(10, true, true);
					cm.全服喇叭(6, "[系统公告]：" + cm.getPlayer().getName() + " 带领队伍开始挑战皮埃尔，大家一起为他（她）鼓掌！");
					cm.dispose();
