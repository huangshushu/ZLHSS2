...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		if (selection == 0) {
			...
			var result = cm.sql_Select("select Name,Win,Lose,Cpoint from memory_hscrank order by Win desc,Lose Asc limit 10;");
...
var Hsc = function () {
	...
	this.del = function (ids) {
		...
		return cm.sql_Update("delete from memory_hsc where id in ? and sponsor = ?", idStr, cm.getPlayer().getId())
	}
