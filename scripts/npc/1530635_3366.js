...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            txt = "#b┏　　　　　　　　" + Icon[17][1] + "  #r#e属性传承#b#n  " + Icon[17][1] + "　　　　　　　　┓#k\r\n\r\n";
            ...
            if (selectedList.length >= 1) {
                ...
                for (var key in selectedList) {
                    ...
                    txt += "\t#fUI/Basic.img/LevelNo/" + itemSeq + "# #r[" + owner + "]#n #d#e" + cm.getItemName(item.getItemId()) + "#n" + itemLevelStr + " #b< " + getState(item) + " > #k\r\n";
                }
                txt += "#k#e\r\n└\t\t\t\t\t\t\t\t\t\t\t┘#n\r\n\r\n";
            }
            //显示计算后的合成成功率以及所需要的费用
            txt += Icon[15][1] + " #d所需的基础费用:[#r " + cost + " #d]\r\n";
            txt += Icon[15][1] + " #d第一个栏放入#r剥离道具#d，第二栏放入#b传承道具#k\r\n";
            txt += Icon[15][1] + " #d道具传承结束 已剥离道具消失 属性将传承到新道具中#k\r\n\r\n";
            //显示确定按钮
            ...
            if (selection == 999) {
                ...
                if (selectedList.length < 2) {
                    ...
                    //设置装备属性
                    toDrop.setState(masterItem.getState(false), false);
                    toDrop.setPotential(masterItem.getPotential(1, true), 1, true);
                    toDrop.setPotential(masterItem.getPotential(2, true), 2, true);
                    toDrop.setPotential(masterItem.getPotential(3, true), 3, true);
                    toDrop.setPotential(masterItem.getPotential(1, false), 1, false);
                    toDrop.setPotential(masterItem.getPotential(2, false), 2, false);
                    toDrop.setPotential(masterItem.getPotential(3, false), 3, false);
                    toDrop.setLimitBreak(masterItem.getLimitBreak());
                    //toDrop.setLevel(masterItem.getLevel());
                    toDrop.setStr(masterItem.getStr());
                    toDrop.setDex(masterItem.getDex());
                    toDrop.setInt(masterItem.getInt());
                    toDrop.setLuk(masterItem.getLuk());
                    toDrop.setHp(masterItem.getHp());
                    toDrop.setMp(masterItem.getMp());
                    toDrop.setWatk(masterItem.getWatk());
                    toDrop.setMatk(masterItem.getMatk());
                    toDrop.setWdef(masterItem.getWdef());
                    toDrop.setMdef(masterItem.getMdef());
                    toDrop.setAcc(masterItem.getAcc());
                    toDrop.setAvoid(masterItem.getAvoid());
                    toDrop.setHands(masterItem.getHands());
                    toDrop.setSpeed(masterItem.getSpeed());
                    toDrop.setJump(masterItem.getJump());
                    toDrop.setUpgradeSlots(masterItem.getUpgradeSlots());
                    //toDrop.setViciousHammer(masterItem.getViciousHammer());
                    toDrop.setEnhance(masterItem.getEnhance());
                    //toDrop.setPVPDamage(masterItem.getPVPDamage());
                    //toDrop.setEnhanctBuff(masterItem.getEnhanctBuff());
                    toDrop.setOwner(masterItem.getOwner());
                    //toDrop.setKarmaCount(masterItem.getKarmaCount());
                    //toDrop.setLock(toDrop);
                ...
                if (selectedPosition != 0 && selectedList[0] == null) {
                    ...
                    while (itemList.hasNext()) {
                        ...
                        if (getReqLevel < 150 && getItemType(item.getItemId()) != RING) continue;
                        //副装备过滤过程
                        if (selectedPosition != 0) {
                            //过滤类型
