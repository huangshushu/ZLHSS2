...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == 180000001) {
		...
		cm.sendGetNumber(""+PPO+" 请输入您想更换的 #r#e发型代码\r\n ",1,00030000,00048207);
    } else if (status == 1) {
		QQL = selection
		cm.sendYesNo("#b您输入代碼的发型为\r\n\r\n#i"+selection+"##r#z"+selection+"#\r\n\r\n#b#e是否进行变更\r\n");
 
}   else if(status == 2){			
	if(selection == null){
		cm.playerMessage(1,"您输入的发型不存在");
		cm.dispose();
		return;
	}
    ...
    cm.setAndroidHair(QQL);
	cm.gainNX(1,-500000);
	cm.sendOk("帮您机器人剪好头发了唷!");
	cm.dispose();
	}
