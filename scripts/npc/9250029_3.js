	...
	if (mode == -1) {
		...
		//	var 爱心 = "#fEffect/CharacterEff/1022223/4/0#";
			...
			if (!cm.haveItem(4001226,1) || !cm.haveItem(4001227,1) || !cm.haveItem(4001228,1) || !cm.haveItem(4001229,1) || !cm.haveItem(4001230,1)) {
				cm.sendOk("#b对不起,您强化材料数量不足！#l");
				status = -1;
			} else if (cm.getBossLog("xhql1") >= 20) {
				//cm.setBossLog("klgw1");
				...
				if (sj == 0) {
					...
					var 力量随机=Math.floor(Math.random()*2+2);
                    var 敏捷随机=Math.floor(Math.random()*2+2);
                    var 运气随机=Math.floor(Math.random()*2+2);
                    var 智力随机=Math.floor(Math.random()*2+2);
				    var 物攻随机=Math.floor(Math.random()*2+2);
                    var 魔攻随机=Math.floor(Math.random()*2+2);
