...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			text += "\t\t\t\t#e#b  " + cm.开服名称() + "2000元赞助礼包 #k	#n\r\n"
			text += "#b\t#v1012057# 全属双攻 +10  #l\r\n"//3
            text += "#b\t#v2340000#[祝福卷轴] x10#l\r\n"//3
			text += "#b\t#v2049100# [混沌卷轴] x20#l\r\n"//3
		  ...
		  if(cm.读取累计赞助()>1999 && cm.getPlayer().getOneTimeLoga("2000元累计赞助礼包") == 0){
			cm.给属性装备(1012057, 0, 0, 10, 10, 10, 10, 0, 0, 10, 10, 0, 0, 0, 0, 0, 0);//V2勋章
				cm.gainItem(2340000, 10);//祝福
				cm.gainItem(2049100,20);
				
		cm.sendOk("领取成功！");
			cm.全服黄色喇叭("[累计充值] : 恭喜玩家 "+cm.getPlayer().getName()+" 成功领取了2000元累充礼包。")
			cm.getPlayer().setOneTimeLoga('2000元累计赞助礼包');
            cm.dispose();
			}else{
