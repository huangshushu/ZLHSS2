...
var 可交易列表 = Array(1112795, 1012349, 1032174, 1022165, 1012350, 1102799, 1003209, 1004387);
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
            ...
            var   text = "#r使用200点券就可以随机获得百宝箱里的礼物，快行动吧！\r\n#e#d" + 小烟花 + "任何建议，请联系管理，谢谢！" + 小烟花 + "#n#k\r\n\r\n";
        text += "#L1#" + 小烟花 +  "使用200点券抽#r1#k次#l#L10"+"#" + 小烟花 + "使用2000点券抽#r10#k次\r\n\r\n";
        ...
        if (selection == 102) {
            ...
            var textItem = ""
             for (var i = 0; i < 可交易列表.length; i++) {
               textItem += "#i" + 可交易列表[i] + ":#"
             }
			textItem += "\r\n";
            cm.sendOk("使用200点券随机获得以下道具，快行动吧！\r\n#e#r任何建议，请联系管理，谢谢！#n#k\r\n" + textItem);
            status = -1;
            //cm.safeDispose();
           ...
           if  (cm.getPlayer().getNX() < 200 ){
			...
			for(var i = 1;  i <= selection; i++){
				...
				var itemid = cm.抽奖(抽奖系列, "装备百宝箱");
				var 数量 = 1;
				if (itemid != -1) {
					var 是否包含 = false;
					for(var j = 0; j < 可交易列表.length; j++){
						if(可交易列表[j] == itemid){
							var item = cm.getNewEquip(itemid);
							item.setFlag(0);
							Packages.server.MapleInventoryManipulator.addFromDrop(cm.getC(), item, false);
							是否包含 = true;
							//cm.getPlayer().dropMessage(1, "检测到" + itemid + "包含！");//测试
							break;
						}
					}
					if(!是否包含){
