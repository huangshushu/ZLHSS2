    ...
    if (status == 0) {
        ...
        if (cm.getAndroidStat("GENDER") == 0) {
            if (selection == 0) {
                ...
                cm.sendAStyle("我可以帮您的安卓换成全新的发型。您厌倦了现在的安卓发型了吗？只要您有20元宝，我就可以帮您的安卓更换发型。请慢慢挑选自己喜欢的发型～", hairnew, 5150052);
            } else if (selection == 2) {
                beauty = 3;
                haircolor = Array();
                ...
                cm.sendAStyle("可以让您的安卓面容焕然一新…不想让您的安卓面容变一变吗？ 只需要20元宝，就可以给您的安卓做整形手术.怎么样~慢慢挑选一下您想要的面容~", facenew, 5152057);
            } else if (selection == 4) {
                beauty = 5;
