        ...
        if (status == 0) {
            if (cm.getMap().getId() == 910048100) {
                ...
                if (emgate.getProperty("open") == "false") {//已经关闭入口了
                    if (cm.getPlayer().isGM()) {
                        status = 2;
                        cm.sendYesNo("尊敬的管理员，您想开放XO宾果活动的入口吗？");
                    } else {
                        cm.sendSimple("现在不是活动时间，请稍后再试！\r\n")
                    }
                    return;
                }
                if (cm.getBossLog("XO宾果活动") >= maxPlay) {
                ...
                if (em == null) {
                    ...
                    if (cm.getPlayer().isGM()) {
                        cm.sendSimple("#n#b\r\n\t\t\t\t#e<XO问答活动>#n\r\n\r\n#dXO问答活动就要开始啦！现在还有几分钟的等待时间……\r\n目前的活动状态：" + Eventstatus + "\r\n\r\n#b#e#L0#参加<XO问答活动>。#l#n\r\n\r\n#L1#我想了解一下该活动的说明。#l\r\n\r\n#L3# 关闭活动入口！(GM可见)")
                    }
            ...
            if (selection == 0) {
                if (em.getProperty("start") == "0") {
                    ...
                    cm.setBossLog("XO宾果活动");
                    cm.getMap().startMapEffect("现在有3分钟的时间等候其它玩家，请稍后！", 5121052);
                    cm.sendOk("[欢迎来到XO问答活动！]\r\n大家好，欢迎来到这里！\r\n在这我们将回答二十道问答题，它们涉及到很多方面，但问题只有两种答案，题目出现的时，选择正确答案，站在正确的位置吧！\r\n#e（站在中间的位置不算，将会被视为错误答案）\r\n#n#r 回答正确获得优胜奖5000万破攻 回答错误获得安慰奖3000万破攻，站中间获得蜗牛壳一个！！！");
                } else if (em.getProperty("start") == "1") {//入口已经开放
                    cm.warp(910048100, "sp");
                    cm.setBossLog("XO宾果活动");
                    cm.getMap().startMapEffect("现在有3分钟的时间等候其它玩家，请稍后！", 5121052);
                    cm.sendOk("[欢迎来到XO问答活动！]\r\n大家好，欢迎来到这里！\r\n在这我们将回答二十道问答题，它们涉及到很多方面，但问题只有两种答案，题目出现的时，选择正确答案，站在正确的位置吧！\r\n#e（站在中间的位置不算，将会被视为错误答案）\r\n#n#r 回答正确获得#i2614069#X1 回答错误获得#i2614068#X1");
                } else {//等待状态
                    cm.warp(910048100, "sp");
                    cm.setBossLog("XO宾果活动");
                    cm.sendOk("[欢迎来到XO问答活动！]\r\n大家好，欢迎来到这里！\r\n在这我们将回答二十道问答题，它们涉及到很多方面，但问题只有两种答案，题目出现的时，选择正确答案，站在正确的位置吧！\r\n#e（站在中间的位置不算，将会被视为错误答案）\r\n#n#r 回答正确获得#i2614069#X1 回答错误获得#i2614068#X1");
                ...
                cm.sendOk("已经关闭了入口！");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经关闭了活动入口（每周二四六开放，请下次再来）。");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经关闭了活动入口（每周二四六开放，请下次再来）。");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经关闭了活动入口（每周二四六开放，请下次再来）。");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经关闭了活动入口（每周二四六开放，请下次再来）。");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经关闭了活动入口（每周二四六开放，请下次再来）。");
                ...
                cm.sendOk("已经开启入口！");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经开启了活动入口（一线白龙进入，三分钟倒计时）。");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经开启了活动入口（一线白龙进入，三分钟倒计时）。");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经开启了活动入口（一线白龙进入，三分钟倒计时）。");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经开启了活动入口（一线白龙进入，三分钟倒计时）。");
				cm.getPlayer().dropMessage(-1, "[XO宾果活动] 管理员已经开启了活动入口（一线白龙进入，三分钟倒计时）。");
