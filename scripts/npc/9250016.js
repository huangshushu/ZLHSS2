1var MapleInventoryManipulator = Java.type('server.MapleInventoryManipulator');
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			cm.sendNext("#r您现在所选择的是#d[万圣节活动帽子]#v1003391##k#l\r\n#d[万圣节活动帽子强化]#b强化成功率:#r100%#k\r\n#b属性幅度:#r-2~+10#k\r\n#b强化消耗:#v2022256##r*10#k\r\n#d次数限制:装备限制上限 #r[30] #k次#l\r\n#bPS：#d由于星之力的魔化产生了不可描述的变异,所以该强化系统无论强化成功或失败,均不会扣除强化次数!\r\n");
		} else if (status == 1) {
		...
		//	var 爱心 = "#fEffect/CharacterEff/1022223/4/0#";
			...
			if (!cm.haveItem(2022256,10)&&cm.getMeso()>=500000) {
				cm.sendOk("#b对不起,邮票数量不足:#r[1]颗！#k\r\n\t   #b当前所拥有#v2022256#:#r [#c2022256#] 颗!#l");
				status = -1;
			} else if (cm.getBossLog("南瓜") >= 1000) {
				cm.setBossLog("南瓜");
				cm.sendOk("你今天已经1000次强化过了,请明天在来吧!");
				status = -1;
			...
			//} else if (cm.getInventory(1).getItem(1).getExpiration() != -1) {
				...
				if (sj == 0) {
					...
					var 力量随机=Math.floor(Math.random()*10);
                    var 敏捷随机=Math.floor(Math.random()*10);
                    var 运气随机=Math.floor(Math.random()*10);
                    var 智力随机=Math.floor(Math.random()*10);
				    var 物攻随机=Math.floor(Math.random()*10);
                    var 魔攻随机=Math.floor(Math.random()*10);
					...
					Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(11,cm.getC().getChannel(),"万圣节活动帽子" + " : " + cm.getPlayer().getName() +"玩家成功 万圣节活动帽子 属性提升。O(∩_∩)O~",true).getBytes());
				...
				//cm.laba(cm.getPlayer().getName() + "『强化公告』" + " : " + "在拍卖-强化成功！大家一起恭喜他（她）吧！", 9);
					...
					Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(11,cm.getC().getChannel(),"万圣节活动帽子" + " : " + cm.getPlayer().getName() +"玩家失败 万圣节活动帽子 属性减少。O(∩_∩)O~",true).getBytes());
