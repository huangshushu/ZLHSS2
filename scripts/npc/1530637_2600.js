...
selects = selection;
			text = "#e#r#h0#,你好！在这里可以用胡萝卜购买物品.请选择您要换购的物品#n\r\n\r\n#e#d目前拥有超级魔方#v4000325#：#d" + cm.getItemQuantity(4000325) + " #k个\r\n#b";
 text += "" + tz17 + "" + tz17 + "" + tz17 + "" + tz17 + "" + tz17 + "" + tz17 + "" + tz17 + "\r\n"
            var x = 0;

			for (var i=0; i<itemlist.length; i++) {
				text += "#L" + i + "# #i" + itemlist[i] + "# #b#z" + itemlist[i] + "#   #r" +  itemlist[i][1] + "胡萝卜 \r\n";
				if (i != 0 && (i+1) % 5 == 0) {
					text += "\r\n";
				}
			}
			...
			selects = selection;
            cm.sendGetNumber("请输入你请你输入想要购买的数量,注意背包格子够不够不然买不到扣物品\r\n\r\n#r - 1个需要 " + itemlist[selects][1] + "胡萝卜", 0, 0, 100);
        } else if (a == 2) {
            buynum = selection;
            cm.sendNext("你想购买" + buynum + "个#r#i" + itemlist[selects][0] + "##k？\r\n你将使用掉" + (buynum * itemlist[selects][1]) + "胡萝卜。");
        } else if (a == 3) {
            if (cm.haveItem(4000325, buynum * itemlist[selects][1])) {
                cm.gainItem(4000325, -buynum * itemlist[selects][1]);
                cm.gainItem(itemlist[selects][0], buynum);
                cm.sendOk("购买成功了！");
                cm.dispose();
            } else {
                cm.sendOk("对不起，你没有足够的超级魔方。");
                cm.dispose();
            }
        }
    }//mode
