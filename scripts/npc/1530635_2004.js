...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			var text = "";
			text = "#fMob/1210102.img/move/0##b月光冒险岛 - MS#fMob/1210102.img/move/0##k\r\n部分技能收取金币,请了解后在进行学习。\r\n";
			text += "#L14##r了解以下技能特效(#e新人必看#n)#l\r\n";
			//#L1##b学习骑术#l
			text += "			#L3##b学习锻造#l #L5##b学习匠人之魂#l\r\n";//"#L2##b学习群宠#l
			text += "#L2##b学习群宠#l\r\n";//"#L2##b学习群宠#l
            ...
            /*if (cm.getPlayer().getSkillLevel(8) > 0 || cm.getPlayer().getSkillLevel(10000018) > 0 || cm.getPlayer().getSkillLevel(20000024) > 0 || cm.getPlayer().getSkillLevel(20011024) > 0) {
                cm.sendOk("您已经学习过这个技能。", 0);
            }else*/
			if (cm.getPlayer().getLevel() <= 50) {
                cm.sendOk("您现在的等级条件还不能学习群宠技能,需要50级才可以学习。", 0);
            } else { //冒险家
                cm.teachSkill(8, 1, 1);
                cm.teachSkill(20011024, 1, 1);
                cm.teachSkill(10000018, 1, 1);
                cm.teachSkill(20000024, 1, 1);
				cm.teachSkill(40011024, 1);
                cm.teachSkill(40021024, 1);
                cm.sendOk("恭喜您学习技能成功,祝您游戏愉快。", 0);
            }
            cm.dispose();
		} else if (selection == 3) {
			...
			if (cm.getMeso() <= 50000000) {
				cm.sendOk("由于英雄之回声技能的特殊性需要支付#b5000万金币#k才可以学习,您目前没有足够的金币。");
			...
			if (cm.getMeso() <= 50000000) {
				...
				cm.sendOk("恭喜您学习技能成功,祝您游戏愉快。");
				} else if (cm.getJobId() >= 4100 && cm.getJobId() <= 4112) {//剑豪
				cm.gainMeso( - 5000000);
				cm.teachSkill(20030190, 1, 1);
				cm.sendOk("恭喜您学习技能成功,祝您游戏愉快。");
				} else if (cm.getJobId() >= 100 && cm.getJobId() <= 512) {//冒险家
			...
			if (cm.getMeso() <= 1000000) {
			   ...
			   cm.sendOk("您现在的等级条件还不能学习飞翔技能,需要120级才可以学习。"); 
				...
				cm.sendOk("恭喜您学习技能成功,祝您游戏愉快。");
				} else if (cm.getJobId() >= 570 && cm.getJobId() <= 572) {//龙人
				cm.gainMeso( - 1000000);
				cm.teachSkill(20031026, 1, 1);
				cm.sendOk("恭喜您学习技能成功,祝您游戏愉快。");
				} else if (cm.getJobId() >= 4100 && cm.getJobId() <= 4112) {//剑豪
				cm.gainMeso( - 1000000);
				cm.teachSkill(40011026, 1, 1);
				cm.sendOk("恭喜您学习技能成功,祝您游戏愉快。");
				} else if (cm.getJobId() >= 3100 && cm.getJobId() <= 3112) {// 恶魔猎手
				cm.gainMeso( - 1000000);
