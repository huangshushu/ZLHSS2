	...
	if (mode == -1) {
		...
		if (a == -1) {
			...
			if (selection == 0) {
				...
				if(cm.getPlayer().getOneTimeLoga("透明" + cm.getInventory(1).getItem(1).getItemId() + "3阶") > 0){
				...
				cm.dispose();
				} else if (cm.haveItem(4310081, 10) && cm.haveItem(4031997, 40) && cm.getPlayer().getCSPoints(1) >= 15000 && cm.getPlayer().getCSPoints(2) >= 5000 && cm.getPlayer().getMeso() >= 30000000 && cm.getInventory(1).getItem(1).getOwner() == "第二阶段") {
					cm.getPlayer().setOneTimeLoga("透明" + cm.getInventory(1).getItem(1).getItemId() + "3阶");
					...
					//cm.全服道具公告("【透明强化】", "玩家 ["+cm.getName()+"]成功将透明装备强化为第三阶段！恭喜恭喜！", item);
					cm.dispose();
				} else {
