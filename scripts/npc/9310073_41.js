    ...
    if (mode == -1) {
        ...
        if (status == 0) {
          ...
          //  text += ""+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+爱心+"\r\n\r\n"
			...
			cm.sendOk("#e独家转生系统,让你越肝越强！\r\n#b转生说明：变成200级保留所有属性和键盘技能\r\n（#rPS：转生等级+1级）#k");

       } else if (selection == 1) {//转生
	        if (cm.getClient().getPlayer().getBeans() > 15) {  
            cm.sendOk("转生等级最高15飞，您已经上天入地无所不能！");
                cm.dispose();
         }else   if (cm.getLevel() < 250) {  
            cm.sendOk("转生要求等级达到250级。您的能力没有资格转生");
                cm.dispose();
				}else if(cm.getPlayer().getMeso() < 200000000){
	            cm.sendOk("需要2E金币才能转生.");
                cm.dispose();
				}else if(!cm.haveItem(4000463,25)){
				   cm.sendOk("需要中介币*25！");
                   cm.dispose();
              }else{
	        cm.getChar().setLevel(201);
			cm.getPlayer().fakeRelog();
			cm.gainMeso(-200000000);//扣除金币
			cm.gainItem(4000463,-25);//扣除道具
			cm.gainItem(5064300,1);
			cm.getPlayer().setBossLog("FSCS",1,1)
		  // cm.gainAp(+5);//转生奖励
