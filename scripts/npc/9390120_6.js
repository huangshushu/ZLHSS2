...
{
	var nextmap = cm.getC().getChannelServer().getMapFactory().getMap(863010240);
	if (mode == -1)
	{
		cm.dispose();
	}
	...
	if (status == 0)
	{	
			if (cm.getMonsterCount(863010240) > 0){
			cm.sendOk("请击败贝勒德的肚脐..");
			cm.dispose();
			}else{
...
if(cm.getMonsterCount(863010240) <= 999){
		

	
}			
			
                           cm.warpParty(863000920,0);
