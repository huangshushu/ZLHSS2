	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if(selection == 0){		
				...
				text1 += "玩法说明：";
				text1 += " #fUI/UIWindow/Quest/icon3/6#\r\n";
				text1 += "    庄家的点数会在16点-22点之间（庄家也有可能爆掉），你有4次要牌的机会，如果您的点数大于庄家的点数即赢得你下注的金额，如果大于21点或者小于等于庄家的点数，即输掉你所下注的金额，10、J、Q、K都算十点，小赌怡情，大赌伤人，请慎重。";
				cm.sendOk(text1);
				cm.dispose();
			}
		}
		//-------------第一次要牌-----------------//
			...
			if(selectionMeso > score){
				...
				zhuangScore = parseInt(Math.random()*5+17); 
		...
		//------------第二次要牌------------------//
			...
			if(selection == 2){
				...
				if((xianScore<22 && xianScore<=zhuangScore && zhuangScore<22) || (xianScore>=22 && zhuangScore>=22)){
					...
					Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(9,cm.getC().getChannel(),cm.getPlayer().getName() + "『赌场公告』" + " : " + "在21点赌场获胜，大家一起祝贺他吧!",true).getBytes()); 
