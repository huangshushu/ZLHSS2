...
function action(mode, type, selection) {
	...
	if (cm.haveItem(1022168,1)) {
	    cm.sendOk("做好了。");
	} else if (!cm.canHold(1022168,1)) {
	    cm.sendOk("请空出一些装备拦空间。");
	} else if (cmp != null && parseInt(cmp) >= 50) {
	    if (cm.getPlayer().getOneInfo(1202, "have") == null || cm.getPlayer().getOneInfo(1202, "have").equals("0")) {
	    	cm.gainItem(1022168, 1, true); //should handle automatically for "have"
	    } else {
		cm.sendOk("你已经有#t1022168#了.");
	    }
	} else {
	    cm.sendOk("你还没有做50次PQ 目前做了: " + (cmp == null ? "0" : cmp) + "次");
	}
	cm.dispose();
