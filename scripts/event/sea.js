...
function beginQuest(eim) {//开始步骤
		var map = eim.getMapInstance(865010004);
		em.getMapFactory().getMap(865010004).startMapEffect("欢迎来到航海王副本!", 5121036);
...
function EndThisBattle(eim) {//结束离开功能
	for (var i = 0; i < eim.getPlayerCount(); i++) {
        eim.getPlayers().get(i).openNpc(0, "hanghaiwang");//奖励
        eim.getPlayers().get(i).dropMessage(1, "[航海王] 挑战成功！");
