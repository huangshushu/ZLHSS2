...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("aka");
    var map = eim.setInstanceMap(272030400);
    map.resetFully();
    map.killAllMonsters(false);
    var mob = em.getMonster(8860000);//190J
    mob.getStats().setChange(true);
    mob.changeLevel(170);
    mob.getChangedStats().setOHp(12600000000);
    mob.setHp(12600000000);
    /*var modified = em.newMonsterStats();
     modified.setOHp(mob.getMobMaxHp() * 300);
     modified.setOMp(mob.getMobMaxMp() * 9000);
     mob.setOverrideStats(modified);*/
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-12, -181));
...
function playerDisconnected(eim, player) {
    eim.disposeIfPlayerBelow(0, 272030400);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
    return 0;
}
...
function monsterValue(eim, mobId) {
    switch (mobId) {
        case 8860000:
            eim.setProperty("MiA", Math.floor((TimeA - eim.getTimeLeft()) / (480 * 1000)));
            eim.setProperty("MiX", Math.floor((TimeA - eim.getTimeLeft()) % (480 * 1000) / 1000));
...
function scheduledTimeout(eim) {
    eim.disposeIfPlayerBelow(100, 272030400);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
}
...
function checkChrHP(eim) {
    ...
    if (state > 0) {
        var map = eim.getMapInstance(0);
        var mob = map.getMonsterById(8860000);
        if (mob != null) {
            for (i = 0; i < eim.getPlayers().size() ; i++) {
