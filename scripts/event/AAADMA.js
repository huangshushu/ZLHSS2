...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(350144000);
    map.resetFully();
    map.killAllMonsters(false);
    eim.startEventTimer(TimeA);

    var mob = em.getMonster(8880101);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(120000000000000);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(615, 16));
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 8880101://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            ...
            C_THREAD(eim, player, 30 * 60 * 1000, "戴米安", 282);
            var map = eim.getMapInstance(0);
