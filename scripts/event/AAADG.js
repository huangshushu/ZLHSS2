...
function setup(level, leaderid) {
    ...
    var mob = em.getMonster(9833674);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(11929920000000000);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-489, 90));
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 9833674://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 10 * 60 * 1000, "神话副本", 282);
            var map = eim.getMapInstance(0);
