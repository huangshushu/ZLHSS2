...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(106031304);
    map.resetFully();
    map.killAllMonsters(false);
    eim.startEventTimer(TimeA);

    var mob = em.getMonster(9303139);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(25000000000000000);
    Attribute.setOMp(mob.getMobMaxMp());
    stats = mob.getStats();
    stats.setPhysicalAttack(1000);
    stats.setMagicAttack(1000);;
    stats.setPDRate(1000);
    stats.setMDRate(1000);
    stats.setAcc(500000);
    stats.setSpeed(200);
    stats.setLevel(250);
    stats.setExp(1000);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(210, 202));
    return eim;
}
...
function playerRevive(eim, player) {
    var map = player.getReviveCount() > 0 ? player.getMap() : eim.getMapFactoryMap(9100000000);
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 100100://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 5 * 60 * 1000, "无尽180-200层", 282);
            var map = eim.getMapInstance(0);
