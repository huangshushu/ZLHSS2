...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(450008650);
    map.resetFully();
    map.killAllMonsters(false);
    eim.startEventTimer(TimeA);

    var mob = em.getMonster(8880312);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(1910105000000000);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-57, 281));
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 8880312://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            ...
            C_THREAD(eim, player, 60 * 60 * 1000, "觉醒威尔", 282);
            var map = eim.getMapInstance(0);
