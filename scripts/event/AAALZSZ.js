...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(910196007);
    map.resetFully();
    map.killAllMonsters(false);
    eim.startEventTimer(TimeA);
	
	var mob = em.getMonster(9390500);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(3000000000000000);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(160, 166));
	
	var mob = em.getMonster(9390501);
    Attribute = em.newMonsterStats();
    ...
    mob.setOverrideStats(Attribute);
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-573, 166));
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 9390500://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 10 * 60 * 1000, "两只狮子", 282);
            var map = eim.getMapInstance(0);
