...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(321190500);
    map.resetFully();
    map.killAllMonsters(false);
    eim.startEventTimer(TimeA);

    var mob = em.getMonster(8644631);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(5000000000000000);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(258, 179));
    ...
    mob.setOverrideStats(Attribute);
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-521, 179));
	
	var mob = em.getMonster(9601050);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(20000000000000000);
    Attribute.setOMp(mob.getMobMaxMp());
    stats = mob.getStats();
    stats.setPhysicalAttack(100);
    stats.setMagicAttack(100);;
    ...
    mob.setOverrideStats(Attribute);
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-138, 179));
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 9601050://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 10 * 60 * 1000, "挑战心魔", 282);
            var map = eim.getMapInstance(0);
