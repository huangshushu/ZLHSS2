...
function setup(level,eim, leaderid) {
    ...
    var eim = em.newInstance("BossWerRe_PuTong");
    var map = eim.setInstanceMap(450008450);
	var map1 = eim.setInstanceMap(450008550);
	var map2 = eim.setInstanceMap(450008650);
    map.resetFully();
	map1.resetFully();
	map2.resetFully();
...
function changedMap(eim, player, mapid) {
    if (mapid != 450008450 && mapid != 450008550 && mapid != 450008650) {
...
function end(eim) {
		...
		var map = eim.getMapInstance(450008450);//setInstanceMap
...
function monsterSpawn(eim) {
		...
		var map2 = eim.getMapInstance(450008650);
	 	...
	 	if (em.getProperty("state").equals("1")==true){
    		...
    		for (var i = 0; i < 1; i++) {
         	...
         	var mob = em.getMonster(8880310);//怪物
         	
            //eim.broadcastPlayerMsg(-6,"巨大黑暗灵魂 HP 检测 "+hprand+"");//检测怪物血量
         	overrideStats.setOHp(110105000000000);
         	overrideStats.setOExp(50000);
         	overrideStats.setOMp(200000);
         	mob.setOverrideStats(overrideStats);
         	mob.setHp(110105000000000);
        
         	eim.registerMonster(mob);

         	map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-31, 159)); //刷出这个
			}
			map.startMapEffect("“威尔”第一阶段已出现，请尽快消灭！", 5120116);
			eim.schedule("summonFall", 5000);    
			eim.setProperty("HPstate", "1");
			em.setProperty("state", "2");
         	...
         	var overrideStats = em.newMonsterStats();
    		for (var i = 0; i < 1; i++) {
         	var mob = em.getMonster(8880311);//怪物
         	var hprand = 4039110001000;
         	var EXPrand = 50000;
         	overrideStats.setOHp(360000000000000);
         	overrideStats.setOExp(EXPrand);
         	overrideStats.setOMp(200000);
         	mob.setOverrideStats(overrideStats);
         	mob.setHp(360000000000000);
         	eim.registerMonster(mob);
         	map1.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-31, 159)); //刷出这个怪物
		        }
			map.startMapEffect("请点击右侧NPC进入下一阶段。", 5120116);
			...
			em.setProperty("state", "3");
         	...
         	var overrideStats = em.newMonsterStats();
    		for (var i = 0; i < 1; i++) {
         	var mob = em.getMonster(8880312);//怪物
         	var hprand = 6039110001000;
         	var EXPrand = 100000;
         	overrideStats.setOHp(1440000000000000);
         	overrideStats.setOExp(EXPrand);
         	overrideStats.setOMp(200000);
         	mob.setOverrideStats(overrideStats);
         	mob.setHp(1440000000000000);
         	eim.registerMonster(mob);
         	map2.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-31, 159)); //刷出这个怪物
		        }
			map1.startMapEffect("点击后侧NPC，挑战最终形态！", 5120116);
		...
		map.broadcastMessage(em.getClock(120));
        	//em.getMapFactory().getMap(450008450).startSimpleMapEffect("1分钟中后将关闭次元世界，请尽快领取点击NPC领取奖励后退出",5120093);
			}
			 }
...
function beginQuest(eim) {
		var map = eim.getMapInstance(450008450);
		//em.getMapFactory().getMap(450008450).startMapEffect("10秒后:将出现巨大X光线将出现!请尽快消灭他！戴米安将会出来复仇", 5120116);
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 8880312://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, TimeA, "觉醒威尔", 282);
            break;
    }
}
/* 下面的内容要进行复制的 */
