...
function setup(level, leaderid) {
    ...
    var mapForMob = eim.getMapInstance(321190330);
    mapForMob.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-10, -40));

    mobid = 8810100;//进阶暗黑龙王左侧头颅
    mob = em.getMonster(mobid);
    mob.changeLevel(275);
    mob.getChangedStats().setOHp(500000000000000000);
    mob.setHp(500000000000000000);
    eim.registerMonster(mob);
    var mapForMob = eim.getMapInstance(240060001);
    mapForMob.spawnMonsterOnGroundBelow(mob, new java.awt.Point(960, 230));
	
	mobid = 8810101;//进阶暗黑龙王右侧头颅
    mob = em.getMonster(mobid);
    mob.changeLevel(275);
    mob.getChangedStats().setOHp(500000000000000000);
    mob.setHp(500000000000000000);
    eim.registerMonster(mob);
    var mapForMob = eim.getMapInstance(240060101);
    mapForMob.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-420, 230));

    mobid = 8810103;//进阶暗黑龙王头1
    mob = em.getMonster(mobid);
    mob.changeLevel(275);
    mob.getChangedStats().setOHp(2000000000000000000);
    mob.setHp(2000000000000000000);
    eim.registerMonster(mob);
    var mapForMob = eim.getMapInstance(240060201);
    mapForMob.spawnMonsterOnGroundBelow(mob, new java.awt.Point(56, 260));
	
	mobid = 8810102;//进阶暗黑龙王头2
    mob = em.getMonster(mobid);
    mob.changeLevel(275);
	...
	mobid = 8810105;//进阶暗黑龙王左手
    mob = em.getMonster(mobid);
    mob.changeLevel(275);
    mob.getChangedStats().setOHp(101050000000000000);
    mob.setHp(101050000000000000);
    eim.registerMonster(mob);
    var mapForMob = eim.getMapInstance(240060201);
    mapForMob.spawnMonsterOnGroundBelow(mob, new java.awt.Point(56, 260));
	
	mobid = 8810106;//进阶暗黑龙王右手
    mob = em.getMonster(mobid);
    mob.changeLevel(275);
    mob.getChangedStats().setOHp(101050000000000000);
    mob.setHp(101050000000000000);
...
function stage1(eim) {
    var map = eim.getMapInstance(321190330);
    for (var i = 0; i < 1; i++) {
        var mob = em.getMonster(9830007);//一阶段小怪
    ...
    map.startMapEffect("愤怒的红大海兽出现了!", 5120182);
...
function stage2(eim) {
    ...
    for (var i = 0; i < 2; i++) {
        var mob = em.getMonster(9830007);//锁链
        modified = em.newMonsterStats();
        modified.setOHp(1);
        modified.setOMp(mob.getMobMaxMp());
        mob.getStats().setPhysicalAttack(500000);//物理伤害
    ...
    map.startMapEffect("进阶暗黑龙王的左侧头颅出现了！", 5120182);
...
function stage4(eim) {//第二阶段闪电
    ...
    map.startMapEffect("进阶暗黑龙王的右侧头颅出现了！", 5120182);
...
function stage5(eim) {//第三阶段放射物
    ...
    var map = eim.getMapInstance(240060101);
    for (var i = 0; i < 1; i++) {
    ...
    map.startMapEffect("噩梦黑龙降临，颤抖吧！凡人！", 5120182);
}

function stage6(eim) {
    var map = eim.getMapInstance(240060101);
    for (var i = 0; i < 1; i++) {
        var mob = em.getMonster(9830007);
        modified = em.newMonsterStats();
        modified.setOHp(1);
        modified.setOMp(mob.getMobMaxMp());
        mob.getStats().setPhysicalAttack(500000);//物理伤害
...
function stage7(eim) {
    var map = eim.getMapInstance(240060201);
    for (var i = 0; i < 1; i++) {
        var mob = em.getMonster(9833111);
        modified = em.newMonsterStats();
        modified.setOHp(1);
        modified.setOMp(mob.getMobMaxMp());
        mob.getStats().setPhysicalAttack(500000);//物理伤害
...
function stage9(eim) {
    ...
    var map = eim.getMapInstance(240060201);
    map.startMapEffect("[噩梦黑龙] 已通关，10S退出副本", 5120182);
...
function changedMap(eim, player, mapid) {
    ...
    player.dropMessage(6, "[噩梦黑龙] 已退出挑战。");
...
function monsterValue(eim, mobid) {
    ...
    switch (mobid) {
        case 9300516:
            ...
            var mapA = eim.getMapInstance(240060001);
            for (var i = 0; i < eim.getPlayerCount(); i++) {
        ...
        case 8810100:
            ...
            var mapA = eim.getMapInstance(240060101);
            for (var i = 0; i < eim.getPlayerCount(); i++) {
                eim.getPlayers().get(i).changeMap(mapA, mapA.getPortal(0))
            }
            break;
		...
		case 8810101:
            if (em.getProperty("state") == 4) {
                ...
                var mapA = eim.getMapInstance(240060201);
                for (var i = 0; i < eim.getPlayerCount(); i++) {
                    eim.getPlayers().get(i).changeMap(mapA, mapA.getPortal(0))
                }
            }
...
function EndThisBattle(eim) {
    for (var i = 0; i < eim.getPlayerCount(); i++) {
        eim.getPlayers().get(i).dropMessage(1, "[噩梦黑龙] 挑战成功！");
    }
...
function scheduledTimeout(eim) {
    //eim.broadcastPlayerMsg(1, "[噩梦黑龙副本] 真遗憾！已超过限定挑战时间，本次挑战失败！别气馁，期待更加强大的您前来挑战~");
    eim.disposeIfPlayerBelow(100, 101050000);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
}
