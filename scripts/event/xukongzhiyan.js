...
function setup(eim, leaderid) {
    ...
    var eim = em.newInstance("xukongzhiyan");
    var map = eim.setInstanceMap(450009500);
    map.resetFully();
    eim.getMapFactoryMap(450009500).killAllMonsters(false);
    var mob = em.getMonster(8644650);
    mob.getStats().setChange(true);
    mob.changeLevel(275);
    mob.getChangedStats().setOHp(1681010500000000000);
    mob.setHp(1681010500000000000);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(62, -234));
    eim.startEventTimer(TimeA); //20分钟 这是副本时间
    return eim;
}
...
function monsterDamaged(eim, player, mobID, damage) {
    switch (mobID) {
        case 8644650://这个是攻击这个怪物显示伤害 至暗魔晶的BOSSID你自己改上去
            /* 这里是玩家造成伤害实时显示 */
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 8644650://最后要显示输出伤害的怪物id 通畅都是最后的一个boss 一样还是BOSSID
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, TimeA, "至暗魔晶", 282);
            //var map = eim.getMapInstance(0);//这里两句话 // 掉 我记得我给你//掉了啊 这两句话没用 会报错 你要自己//掉
...
function allMonstersDead(eim) {
    ...
    mob.getStats().setChange(true);
    mob.changeLevel(275);
    mob.getChangedStats().setOHp(4200000000000);
    mob.setHp(4200000000000);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(12, 265));
}
