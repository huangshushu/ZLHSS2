...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("jinjieyuwang");
    var map = eim.setInstanceMap(860000041);
    map.resetFully();
    map.killAllMonsters(false);
    var mob = em.getMonster(9390000);//190J
    mob.getStats().setChange(true);
    mob.changeLevel(200);
    mob.getChangedStats().setOHp(22620000000);
    mob.setHp(22620000000);
    /*var modified = em.newMonsterStats();
     modified.setOHp(mob.getMobMaxHp() * 300);
     modified.setOMp(mob.getMobMaxMp() * 9000);
     mob.setOverrideStats(modified);*/
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(663, 167));
...
function playerDisconnected(eim, player) {
    eim.disposeIfPlayerBelow(0, 860000041);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
    return 0;
}
...
function scheduledTimeout(eim) {
    eim.disposeIfPlayerBelow(100, 860000041);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
}
...
function checkChrHP(eim) {
    ...
    if (state > 0) {
        var map = eim.getMapInstance(0);
        var mob = map.getMonsterById(9390000);
        if (mob != null) {
            for (i = 0; i < eim.getPlayers().size() ; i++) {
