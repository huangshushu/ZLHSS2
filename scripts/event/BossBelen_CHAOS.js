...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 8930000://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, TimeA, "贝伦", 282);
            break;
    }
};
...
function allMonstersDead(eim) {
    ...
    mob.getStats().setChange(true);
	    mob.changeLevel(200);
    mob.getChangedStats().setOHp(1500000000000);
