...
function startEvent() {
    ...
    var nextTime = cal.getTimeInMillis();
    while (nextTime <= java.lang.System.currentTimeMillis()) {
        nextTime += 2000 * 25 * 10; //设置多久结束
    }
    setupTask = em.scheduleAtTimestamp("finishEvent", nextTime);
	em.broadcastServerMsg(5121010, " < 召唤的怪物 > 每小时10分:00-15分:00，请到市场雪人NPC参加活动！",true);
    em.broadcastServerMsg("[召唤的怪物]  每小时10分:00-15分:00，请到市场雪人NPC参加活动！");
}

function finishEvent() {
    if (em.getProperty("endEvent").equals("false")) {
        em.broadcastServerMsg(5121010, " < 召唤的怪物 > 每小时40分:00-45分:00，请到市场雪人NPC参加活动！",true);
    } else {
        em.broadcastServerMsg("[召唤的怪物]  每小时40分:00-45分:00，请到市场雪人NPC参加活动！");
    }
    scheduleNew();
}
