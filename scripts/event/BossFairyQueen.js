...
function setup(level, leaderid) {
    ...
    var eim = em.newInstance("ABDyjnh");
    var map = eim.setInstanceMap(300030310);
    map.resetFully();
    map.killAllMonsters(false);
    eim.startEventTimer(TimeA);

    var mob = em.getMonster(5250007);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(100000000000000);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(43, 149));
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 5250007://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 30 * 60 * 1000, "妖精女皇", 282);
            var map = eim.getMapInstance(0);
