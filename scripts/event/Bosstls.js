...
/* 开始地图 退出地图 副本时间 BOSS */
var Group = [867200630, 101050000, 3600000, 9309207]
...
function setup(eim, CharID) {
    ...
    Attribute.setOHp(100000000000000);//血量
    Attribute.setOMp(mob.getMobMaxMp());
    Attribute.setOExp(1000);
    stats = mob.getStats();
    stats.setPhysicalAttack(200);//攻击?
    stats.setMagicAttack(200);//攻击?
    stats.setPDRate(200);//这个好像是防御不百分比
    stats.setMDRate(200);//一样魔防
    stats.setAcc(50000);//闪避
    stats.setSpeed(200);//移速
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(284, 365));
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 9309207://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 3600000, "桃乐丝", 282);
            var map = eim.getMapInstance(0);
