...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(802000825);
    map.resetFully();
    ...
    Attribute.setOHp(50000000000000);
    Attribute.setOMp(mob.getMobMaxMp());
    stats = mob.getStats();
    stats.setPhysicalAttack(150);
    stats.setMagicAttack(150);;
    stats.setPDRate(150);
    stats.setMDRate(150);
    stats.setAcc(500000);
    stats.setSpeed(200);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(8, 335));
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 9400289://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 30 * 60 * 1000, "欧碧拉", 282);
            var map = eim.getMapInstance(0);
