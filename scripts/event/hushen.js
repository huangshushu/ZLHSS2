...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("hushen");
    var map = eim.setInstanceMap(410000124);
    map.resetFully();
    map.killAllMonsters(false);
    var mob = em.getMonster(2400259);//190J
    mob.getStats().setChange(true);
    mob.changeLevel(200);
    mob.getChangedStats().setOHp(8756300000);
    mob.setHp(8756300000);
    /*var modified = em.newMonsterStats();
     modified.setOHp(mob.getMobMaxHp() * 300);
     modified.setOMp(mob.getMobMaxMp() * 9000);
     mob.setOverrideStats(modified);*/
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(530, 158));
...
function playerDisconnected(eim, player) {
    eim.disposeIfPlayerBelow(0, 410000124);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
    return 0;
}
...
function scheduledTimeout(eim) {
    eim.disposeIfPlayerBelow(100, 410000124);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
}
...
function allMonstersDead(eim) {
    ...
    //eim.startEventTimer(12000 * 60); //10 min
...
function checkChrHP(eim) {
    ...
    if (state > 0) {
        var map = eim.getMapInstance(0);
        var mob = map.getMonsterById(2400259);
        if (mob != null) {
            for (i = 0; i < eim.getPlayers().size() ; i++) {
