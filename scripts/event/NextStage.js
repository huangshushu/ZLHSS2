function enter(pi) {
    ...
    if (pi.getPlayer().getMapId() == 744000014 && pi.getPlayer().getParty() != null && !pi.haveMonster(9410183) && pi.isLeader()) {
        ...
        pi.playPortalSE();
	}else if(pi.getPlayer().getMapId() == 744000001 && pi.getPlayer().getParty() != null && !pi.getMonsterCount(744000001)==0 ){
		pi.playerMessage(5, "恭喜您通过了,已经发放奖励！");
		for (var i =0;i<5 ;i++ ){
			if (pi.getPQLog('枫之高校')==i){
				pi.gainItem(4310105,3+(i*2));
				break;
			}
		}
		pi.setPQLog('枫之高校');
		pi.warp(101050000);
		pi.playerMessage(5,"提示：每日可获得次数为5次");
    } else {
        if(pi.getPlayer().getMapId() != 744000008){
