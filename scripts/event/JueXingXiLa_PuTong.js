...
function setup(eim, leaderid) {
    ...
    var eim = em.newInstance("JueXingXiLa_PuTong");
    var map = eim.setInstanceMap(450010700);
    map.resetFully();
    eim.getMapFactoryMap(450010700).killAllMonsters(false);
    var mob = em.getMonster(8880410);
    mob.getStats().setChange(true);
    mob.changeLevel(200);
    mob.getChangedStats().setOHp(100000000000000);
    mob.setHp(100000000000000);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(12, 265));
    eim.startEventTimer(60 * 60 * 1000 * 1); //30分钟
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 8880410://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 60 * 60 * 1000 * 1, "觉醒希拉", 282);
            var map = eim.getMapInstance(0);
...
function allMonstersDead(eim) {
    ...
    mob.getStats().setChange(true);
    mob.changeLevel(200);
    mob.getChangedStats().setOHp(420000000000);
    mob.setHp(420000000000);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(12, 265));
}
