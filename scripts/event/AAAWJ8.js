...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(106031302);
    map.resetFully();
    map.killAllMonsters(false);
    eim.startEventTimer(TimeA);

    var mob = em.getMonster(9303137);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(15000000000000000);
    Attribute.setOMp(mob.getMobMaxMp());
    stats = mob.getStats();
    stats.setPhysicalAttack(800);
    stats.setMagicAttack(800);;
    stats.setPDRate(800);
    stats.setMDRate(800);
    stats.setAcc(500000);
    stats.setSpeed(200);
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 100100://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            ...
            C_THREAD(eim, player, 15 * 60 * 1000, "无尽140-160层", 282);
            var map = eim.getMapInstance(0);
