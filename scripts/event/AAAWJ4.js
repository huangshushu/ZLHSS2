...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(993160125);
    map.resetFully();
    map.killAllMonsters(false);
    eim.startEventTimer(TimeA);

    var mob = em.getMonster(9303133);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(2000000000000000);
    Attribute.setOMp(mob.getMobMaxMp());
    stats = mob.getStats();
    stats.setPhysicalAttack(400);
    stats.setMagicAttack(400);;
    stats.setPDRate(400);
    stats.setMDRate(400);
    stats.setAcc(500000);
    stats.setSpeed(200);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(366, 86));
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 100100://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            ...
            C_THREAD(eim, player, 35 * 60 * 1000, "无尽060-080层", 282);
            var map = eim.getMapInstance(0);
