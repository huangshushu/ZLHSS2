...
function setup(eim, leaderid) {
    ...
    mob.getChangedStats().setOHp(4000000000000);
    mob.setHp(4000000000000);
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 8900000://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 30 * 60 * 1000, "皮埃尔", 282);
            var map = eim.getMapInstance(0);
...
function allMonstersDead(eim) {
    ...
    var map = eim.getMapInstance(0);
	    var spendTime = TimeA - eim.getTimeLeft();
    eim.setProperty("MiA", Math.floor(spendTime / (60 * 1000)));
    eim.setProperty("MiX", Math.floor(spendTime % (60 * 1000) / 1000));
