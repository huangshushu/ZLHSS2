...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(993160126);
    map.resetFully();
    map.killAllMonsters(false);
    eim.startEventTimer(TimeA);

    var mob = em.getMonster(9303134);
    Attribute = em.newMonsterStats();
    Attribute.setOHp(2500000000000000);
    Attribute.setOMp(mob.getMobMaxMp());
    stats = mob.getStats();
    stats.setPhysicalAttack(500);
    stats.setMagicAttack(500);;
    stats.setPDRate(500);
    stats.setMDRate(500);
    stats.setAcc(500000);
    stats.setSpeed(200);
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(394, 86));
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 100100://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            ...
            C_THREAD(eim, player, 30 * 60 * 1000, "无尽080-100层", 282);
            var map = eim.getMapInstance(0);
