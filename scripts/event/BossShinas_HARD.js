...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(271040000);
    map.resetFully();
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-96, 108));
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 8850011://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 30 * 60 * 1000, "希纳斯", 282);
            var map = eim.getMapInstance(0);
