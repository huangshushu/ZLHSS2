...
function setup(chrID, mapID) {
    ...
    var eim = em.newInstance(mapID + "_MapInstance_" + chrID);
...
function monsterKilled(eim, player, mobID) {
    ...
    switch (map.getMapID()) {
        case 913070050:
            if (map.getEventMobSize() <= 0) {
                eim.forceCompleteQuest(player, 20035);
            }
            break;
        case 940011110:
            if (map.getEventMobSize() <= 0) {
                var mmap = em.getMap(940011150);
                player.changeMap(mmap, mmap.getPortal(0));
            }
            break;
    }
}