...
function setup(level, leaderid) {
    ...
    var map = eim.setInstanceMap(106031310);
    map.resetFully();
    ...
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(170, 202));
    return eim;
}
...
function monsterKilled(eim, player, mobID) {
    switch (mobID) {
        case 9300594://最后要显示输出伤害的怪物id 通畅都是最后的一个boss
            /* 这里是打死BOSS执行通关 | eim player | 副本时间 | 副本名称  | 公告类型 */
            C_THREAD(eim, player, 100 * 60 * 1000, "无尽20-40层", 282);
            var map = eim.getMapInstance(0);
