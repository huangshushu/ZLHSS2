...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("beilede");//改------
    var map = eim.setInstanceMap(863000920);//改------
    map.resetFully();
    map.killAllMonsters(false);
    var mob = em.getMonster(9390600);//改------
    mob.getStats().setChange(true);
    mob.changeLevel(180);//改------
    mob.getChangedStats().setOHp(58260000000);//改------
    mob.setHp(58260000000);//改------
    /*var modified = em.newMonsterStats();
     modified.setOHp(mob.getMobMaxHp() * 300);
     modified.setOMp(mob.getMobMaxMp() * 9000);
     mob.setOverrideStats(modified);*/
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(12, 62));//改------
...
function playerDisconnected(eim, player) {
    eim.disposeIfPlayerBelow(0, 863000920);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
    return 0;
}
...
function monsterValue(eim, mobId) {
    switch (mobId) {
        case 9390600:
            eim.setProperty("MiA", Math.floor((TimeA - eim.getTimeLeft()) / (660 * 1000)));
            eim.setProperty("MiX", Math.floor((TimeA - eim.getTimeLeft()) % (660 * 1000) / 1000));
...
function scheduledTimeout(eim) {
    eim.disposeIfPlayerBelow(100, 863000920);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
}
...
function checkChrHP(eim) {
    ...
    if (state > 0) {
        var map = eim.getMapInstance(0);
        var mob = map.getMonsterById(9390600);
        if (mob != null) {
            for (i = 0; i < eim.getPlayers().size() ; i++) {
