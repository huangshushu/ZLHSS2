...
var closeTime = 60000; //船关闭搭乘的时间
var beginTime = 60000; //船启航的时间
// 1秒=1000毫秒10分钟=10*60*1000=600000 毫秒
var rideTime = 200000; //搭船所需时间
...
function init() {
    Orbis_btf = em.getChannelServer().getMapFactory().getMap(240000111); Ellinia_btf = em.getChannelServer().getMapFactory().getMap(200000132);  Boat_to_Orbis = em.getChannelServer().getMapFactory().getMap(200090200);   Boat_to_Ellinia = em.getChannelServer().getMapFactory().getMap(200090210);   Orbis_Boat_Cabin = em.getChannelServer().getMapFactory().getMap(200090011);
   Ellinia_Boat_Cabin = em.getChannelServer().getMapFactory().getMap(200090001);
  Orbis_docked = em.getChannelServer().getMapFactory().getMap(240000100);
    Ellinia_docked = em.getChannelServer().getMapFactory().getMap(200000131);
   Orbis_Station = em.getChannelServer().getMapFactory().getMap(240000110);
 	stopEntryOpen = false;
	takeoffOpen = false;
	arrivedOpen = false;
	invasionOpen = false;
...
function invasion() {
    ...
    //if (numspawn == 2) {
        ...
        for (var i = 0; i < numspawn; i++) {
            Boat_to_Orbis.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9400514), new java.awt.Point(253, 120));
            Boat_to_Ellinia.spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9400514), new java.awt.Point(253, 120));
        }
