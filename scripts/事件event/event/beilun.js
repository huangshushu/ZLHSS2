...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("beilun");//改------
    var map = eim.setInstanceMap(105200410);//改------
    map.resetFully();
    map.killAllMonsters(false);
    var mob = em.getMonster(8930000);//改------
    mob.getStats().setChange(true);
    mob.changeLevel(190);//改------
    mob.getChangedStats().setOHp(45236969600);//改------
    mob.setHp(45236969600);//改------
    /*var modified = em.newMonsterStats();
     modified.setOHp(mob.getMobMaxHp() * 300);
     modified.setOMp(mob.getMobMaxMp() * 9000);
     mob.setOverrideStats(modified);*/
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(-197, 443));//改------
...
function checkChrHP(eim) {
    ...
    if (state > 0) {
        var map = eim.getMapInstance(0);
        var mob = map.getMonsterById(8930000);
        if (mob != null) {
            for (i = 0; i < eim.getPlayers().size() ; i++) {
