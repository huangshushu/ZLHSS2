...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("xuexingnvwang");
    var map = eim.setInstanceMap(105200310);
    map.resetFully();
    map.killAllMonsters(false);
    var mob = em.getMonster(8920002);//190J
    mob.getStats().setChange(true);
    mob.changeLevel(190);
    mob.getChangedStats().setOHp(76720000000);
    mob.setHp(76720000000);
...
function playerDisconnected(eim, player) {
    eim.disposeIfPlayerBelow(0, 105200310);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
    return 0;
}
...
function checkChrHP(eim) {
    ...
    if (state > 0) {
        var map = eim.getMapInstance(0);
        var mob = map.getMonsterById(8920002);
        if (mob != null) {
            for (i = 0; i < eim.getPlayers().size() ; i++) {
