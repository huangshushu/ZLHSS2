...
function setup(eim, leaderid) {
    ...
    em.setProperty("leader", "true");
    var eim = em.newInstance("baojun");//改------
    var map = eim.setInstanceMap(401060200);//改------
    map.resetFully();
    map.killAllMonsters(false);
    var mob = em.getMonster(8880000);//改------
    mob.getStats().setChange(true);
    mob.changeLevel(180);//改------
    mob.getChangedStats().setOHp(42360000000);//改------
    mob.setHp(42360000000);//改------
    /*var modified = em.newMonsterStats();
     modified.setOHp(mob.getMobMaxHp() * 300);
     modified.setOMp(mob.getMobMaxMp() * 9000);
     mob.setOverrideStats(modified);*/
    eim.registerMonster(mob);
    map.spawnMonsterOnGroundBelow(mob, new java.awt.Point(2436, -1347));//改------
...
function playerDisconnected(eim, player) {
    eim.disposeIfPlayerBelow(0, 401060200);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
    return 0;
}
...
function scheduledTimeout(eim) {
    eim.disposeIfPlayerBelow(100, 401060200);
    em.setProperty("state", "0");
    em.setProperty("leader", "true");
}
