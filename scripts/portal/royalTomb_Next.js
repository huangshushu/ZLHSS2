function enter(pi) {
	...
	if(pi.getMap(745010100).isDisconnected(pi.getPlayer().getId())){
			pi.playPortalSE();
			pi.warp(745010100, "sp");
	}
	...
	if (pi.getPlayerCount(745010100) <= 0) {
			var FantMap = pi.getMap(745010100);
			//pi.setBossLog("皇帝");
			FantMap.resetFully();
			pi.playPortalSE();
			pi.warp(745010100, "sp");	
	} else {
	    if (pi.getMap(745010100).getSpeedRunStart() == 0 && (pi.getMonsterCount(745010100) <= 0)) {
		pi.playPortalSE();
		//pi.setBossLog("暴力熊");
		pi.warp(745010100, "sp");
		}else{
