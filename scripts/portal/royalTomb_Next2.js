function enter(pi) {
	...
	if(pi.getMap(745010300).isDisconnected(pi.getPlayer().getId())){
			pi.playPortalSE();
			pi.warp(745010300, "sp");
	}
	...
	if (pi.getPlayerCount(745010300) <= 0) {
			var FantMap = pi.getMap(745010300);
			//pi.setBossLog("皇帝");
			FantMap.resetFully();
			pi.playPortalSE();
			pi.warp(745010300, "sp");	
	} else {
	    if (pi.getMap(745010300).getSpeedRunStart() == 0 && (pi.getMonsterCount(745010300) <= 0)) {
		pi.playPortalSE();
		//pi.setBossLog("暴力熊");
		pi.warp(745010300, "sp");
		}else{
