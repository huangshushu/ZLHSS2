...
function enter(pi) {
    ...
    if (em != null && eim != null) {
        ...
        var rStat = pi.getReactorStat("b") + 1;
        var rValue = parseInt(pi.getQuestInfo(42006, "b"));

        if (rValue > rStat) {//获得的数字 大于当前的数字 则进行处理
            pi.updateOneQuestInfo(42006, "bc", String(rValue));
            pi.showAvatarOrientedRepeat(false, "");
            pi.showAvatarOriented("Effect/OnUserEff.img/aquarisTower/success");
            pi.updateOneQuestInfo(42006, "b", "0");
            pi.forceTrigger("b", rValue - 1);
            ...
            if (!"0".equals(rc) && !"0".equals(gc) && !"0".equals(bc) && !"0".equals(yc)) {
                ...
                eim.broadcastWeatherEffectNotice("你成功用" + (rValue == 8 ? "紫色" : "蓝色" + rValue) + "卡片欺骗了结界！", 147, 15000);
            }
        }
    } else {
