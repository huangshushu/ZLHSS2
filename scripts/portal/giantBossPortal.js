...
function enter(pi) {
	...
	try {
		...
		if (em != null && !pi.getMap().getAllMonster().isEmpty() && (pi.getPortalName().startsWith("in") || pi.getPortalName().startsWith("east") || pi.getPortalName().startsWith("west")) && mapid != 863010100 && mapid != 863010500) {
		...
		if (mapid == 863010100) {
			...
			pi.showMapEffect("giantBoss/enter/11");
			...
			if (pi.getPortal().getId() == 5) {
				...
				pi.showMapEffect("giantBoss/enter/24");
			} else if (pi.getPortal().getId() == 3) {
