 ...
 /*public static class pk extends 猜拳 {
    ...
    public static class online extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(6, new StringBuilder().append("当前").append(c.getChannel()).append("频道: ").append(channelOnline).append("人   ").append("当前服务器总计线上人数: ").append(totalOnline).append("个").toString());
            return true;
        }
    ...
    public static class ea extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            c.sendPacket(MaplePacketCreator.sendHint("解卡完毕", 350, 5));
                           ...
                           // + "经验值倍率 " + ((Math.round(c.getPlayer().getEXPMod()) * c.getPlayer().getExpm() * 100 * c.getChannelServer().getExpRate()) * Math.round(c.getPlayer().getStat().expBuff / 100.0) + (c.getPlayer().getStat().equippedFairy ? c.getPlayer().getFairyExp() : 0))
                           // + "%, 掉宝倍率 " + Math.round(c.getPlayer().getDropMod() * c.getPlayer().getDropm() * (c.getPlayer().getStat().dropBuff / 100.0) * 100 * c.getChannelServer().getDropRate()) + "%, 金币倍率 " + Math.round((c.getPlayer().getStat().mesoBuff / 100.0) * 100 * c.getChannelServer().getMesoRate()) + "% \r\n "
...
//     EnterCashShop
    ...
    /*public static class 匿名广播 extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append("@匿名广播 - 具体用法 “@iii 1/2 具体内容” 1为GASH，2为枫叶点数。").toString();
        }
    }*/
