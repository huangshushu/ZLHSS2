...
public class ConsoleCommand {
    ...
    /*public static class Info extends ConsoleCommandExecute {
        ...
        public int execute(String[] paramArrayOfString) {
            ...
            //System.out.println("SQL连接数 :" + ((Integer) DatabaseConnection.getConnectionsCount()).toString());
                ...
                if (ts == null && (t == null || !t.isAlive())) {
                    ...
                    ts = Timer.EventTimer.getInstance().register(new Runnable() {
                        ...
                        public void run() {
                            ...
                            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[枫之谷公告] 服务器将在 " + minutesLeft + " 分钟后关闭，请勿关闭精灵商人并存档并下线。."));
 ...
 /*public static class ExpRate extends ConsoleCommandExecute {
    ...
    public static class Online extends ConsoleCommandExecute {
        ...
        public int execute(String[] splitted) {
            ...
            for (ChannelServer ch : ChannelServer.getAllInstances()) {
                System.out.println("----------------------------------------------------------");
                System.out.println(new StringBuilder().append("频道: ").append(ch.getChannel()).append(" 线上人数: ").append(ch.getConnectedClients()).toString());
                ...
                System.out.println(new StringBuilder().append("当前频道总计线上人数: ").append(total).toString());
                System.out.println("-------------------------------------------------------------------------------------");
            }

            System.out.println(new StringBuilder().append("当前服务器总计线上人数: ").append(total).append("个").toString());
            System.out.println("-------------------------------------------------------------------------------------");
    ...
    public static class GMmessage extends ConsoleCommandExecute {
        ...
        public int execute(String splitted[]) {
            ...
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[后台GM广播] " + outputMessage));
            return 1;
        }
