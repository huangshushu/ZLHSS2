...
public class ItemMakerHandler {
    ...
    public static final void ItemMaker(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (b) {
            ...
            if (i < 1 || i > 96) {
                ...
                if (!player.canHold()) {
                    c.getPlayer().dropMessage(5, "空间不足，每个背包至少留出2个空间");
                    return;
                }
    ...
    private final static boolean addById(int[] ints, final MapleClient c) {
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        int i1 = Randomizer.nextInt(ints.length);
        final MapleInventoryType type = GameConstants.getInventoryType(ints[i1]);
        if (type.equals(MapleInventoryType.EQUIP)) {
            IItem iItem = ii.getEquipById(ints[i1]);
            return MapleInventoryManipulator.addFromDrop(c, iItem, true);
        } else {
            final MaplePet pet;
            if (ItemConstants.类型.宠物(ints[i1])) {
                pet = MaplePet.createPet(ints[i1], MapleInventoryIdentifier.getInstance());
            } else {
                pet = null;
            }
            c.sendPacket(MaplePacketCreator.getShowItemGain(ints[i1], (short) 1, true));
            return MapleInventoryManipulator.addById(c, ints[i1], (short) 1, "", pet, 0);
        }
    }
