...
public class PlayersHandler {
    ...
    public static void GiveFame(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (target != null) {
            if (target.getId() == chr.getId()) { // faming self
                FileoutputUtil.logToFile("logs/Hack/Ban/修改封包.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家："
                        + chr.getName() + "(" + chr.getId() + ") 修改名声封包，使用时封锁。加自己名声");
                World.Broadcast.broadcastMessage(
                        MaplePacketCreator.serverNotice(6, "[封锁系统] " + chr.getName() + " 因为修改封包而被管理员永久停权。"));
                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6,
                        "[GM密语]  " + chr.getName() + "(" + chr.getId() + ") 修改名声封包，使用时封锁。加自己名声"));
                chr.ban("修改封包", true, true, false);
                chr.getClient().getSession().close();
                return;
            } else if (chr.getLevel() < 15) {
                FileoutputUtil.logToFile("logs/Hack/Ban/修改封包.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家："
                        + chr.getName() + "(" + chr.getId() + ")(等级:" + chr.getLevel() + ") 修改名声封包，使用时封锁。十五等以下加名声");
                World.Broadcast.broadcastMessage(
                        MaplePacketCreator.serverNotice(6, "[封锁系统] " + chr.getName() + " 因为修改封包而被管理员永久停权。"));
                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语]  " + chr.getName() + "("
                        + chr.getId() + ")(等级:" + chr.getLevel() + ") 修改名声封包，使用时封锁。十五等以下加名声"));
                chr.ban("修改封包", true, true, false);
                chr.getClient().getSession().close();
                return;
            }
    ...
    public static void UnlockItem(final LittleEndianAccessor slea, final MapleClient c) { // 封印之锁解除钥匙 ID:2051000
        ...
        if (add) {
            eqs.put(item, type);
            MapleInventoryManipulator.removeById(c.getPlayer().getClient(), MapleInventoryType.USE, UnlockItem, 1,
                    false, false);
        }
        add = false;
    }
    ...
    public static void RingAction(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (mode == 0) {
            ...
            if (accepted) {
                ...
                chr.getClient()
                        .sendPacket(MaplePacketCreator.sendEngagement((byte) 0x10, newItemId, chr, c.getPlayer()));
    ...
    public static void LieDetector(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr,
        ...
        if (((search_chr.getAntiMacro().isPassed()) && (isItem))
                || (search_chr.getAntiMacro().canDetector(System.currentTimeMillis()))) {
            // if (((search_chr.getAntiMacro().canDetector(System.currentTimeMillis())) &&
            // (isItem))) {
    ...
    public static void LieDetectorResponse(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (answer.equalsIgnoreCase(ld.getAnswer())) {
            ...
            if ((search_chr != null) && (search_chr.getId() != c.getPlayer().getId())) {
                ...
                // c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 没有通过测谎仪。");
    ...
    /*
     * public static void LieDetectorRefresh(LittleEndianAccessor slea, MapleClient
     * c) {
     * if ((c.getPlayer() == null) || (c.getPlayer().getMap() == null)) {
     * return;
     * }
     ...
     * } else {
     * ld.end();
     * c.getPlayer().getClient().getSession().writeAndFlush(MaplePacketCreator.
     * LieDetectorResponse((byte) 7, (byte) 0));
     * MapleMap map = c.getPlayer().getMap().getReturnMap();
     * c.getPlayer().changeMap(map, map.getPortal(0));
     * World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, new
     * StringBuilder().append("[GM密语] 玩家: ").append(c.getPlayer().getName()).
     * append(" (等级 ").append(c.getPlayer().getLevel()).
     * append(") 未通过测谎仪检测，疑似使用脚本外挂！").toString()));
     * }
     * }
     */
}
