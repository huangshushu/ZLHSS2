...
//        }
        ...
        //chr.getClient().getSession().write(MaplePacketCreator.giveMount(tamingMob, itemId, stat));
        //chr.getMap().broadcastMessage(chr, MaplePacketCreator.showChair(chr.getId(), itemId), false);
...
//			chr.getMap().broadcastMessage(chr, MaplePacketCreator.showMagnet(mobId, slea.readByte()), chr.getPosition());
    ...
    public static final void closeRangeAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr, final boolean energy) {
        ...
        if (attack.skill != 0) {
            if (attack.skill == 海盗.双子星攻击 || attack.skill == 枪手.散射 || attack.skill == 枪手.脱离战场 || attack.skill == 神枪手.三连发 || attack.skill == 神枪手.寒霜喷射 || attack.skill == 神枪手.火焰喷射 || attack.skill == 神枪手.指定攻击 || attack.skill == 枪神.瞬迅雷 || attack.skill == 枪神.心灵控制 || attack.skill == 盗贼.双飞斩 || attack.skill == 刺客.吸血术 || attack.skill == 暗杀者.风魔手里剑 || attack.skill == 暗杀者.枫币攻击 || attack.skill == 夜使者.挑衅 || attack.skill == 暗影神偷.挑衅 || attack.skill == 夜使者.三飞闪 || attack.skill == 暗夜行者1.双飞斩 || attack.skill == 暗夜行者3.风魔手里剑 || attack.skill == 暗夜行者3.三飞闪 || attack.skill == 破风使者1.二连箭 || attack.skill == 破风使者3.箭雨 || attack.skill == 破风使者3.四连箭 || attack.skill == 破风使者3.疾风扫射 || attack.skill == 破风使者3.疾风光速神弩 || attack.skill == 破风使者3.暴风神射 || attack.skill == 弓箭手.二连箭 || attack.skill == 弓箭手.断魂箭 || attack.skill == 猎人.炸弹箭 || attack.skill == 游侠.四连箭 || attack.skill == 游侠.烈火箭 || attack.skill == 游侠.箭雨 || attack.skill == 箭神.龙魂之箭 || attack.skill == 箭神.暴风神射 || attack.skill == 弩弓手.穿透之箭 || attack.skill == 狙击手.四连箭 || attack.skill == 狙击手.升龙弩 || attack.skill == 狙击手.寒冰箭 || attack.skill == 神射手.龙魂之箭 || attack.skill == 神射手.必杀狙击 || attack.skill == 神射手.光速神弩) {
                c.getSession().close();
                return;
            }
    ...
    public static final void Heal(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        if (healMP != 0 && chr.canMP()) {
            ...
            if (healMP > check * 5) {
                //  chr.getCheatTracker().registerOffense(CheatingOffense.REGEN_HIGH_MP, String.valueOf(healMP) + " 服务端:" + stats.getHealMP());
