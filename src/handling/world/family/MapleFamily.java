...
public final class MapleFamily implements java.io.Serializable {
    ...
    public MapleFamily(final int fid) {
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            if (leadername == null || members.size() < 2) {
                System.err.println(
                        "Leader " + leaderid + " isn't in family " + id + ".  Impossible... family is disbanding.");
                writeToDB(true);
                proper = false;
                return;
            }
    ...
    public final int setRep(final int cid, int addrep, final int oldLevel) {
        ...
        if (mgc != null && mgc.getFamilyId() == id) {
            ...
            if (mgc.isOnline()) {
                ...
                setOfflineFamilyStatus(id, mgc.getSeniorId(), mgc.getJunior1(), mgc.getJunior2(),
                        mgc.getCurrentRep() + addrep, mgc.getTotalRep() + addrep, mgc.getId());
            }
    ...
    /*
     * public final void leaveFamily(final MapleFamilyCharacter mgc, final boolean
     * skipLeader) {
     * bDirty = true;
     ...
     * //toSplit = initiator who either broke off with their junior/senior, splitId
     * is the ID of the one broken off
     ...
     * }
     * try {
     * List<MapleFamilyCharacter> all = leader.getAllJuniors(this); //leader is
     ...
     * setOfflineFamilyStatus(newId, mgc.getSeniorId(), mgc.getJunior1(),
     * mgc.getJunior2(), mgc.getCurrentRep(), mgc.getTotalRep(), mgc.getId());
     ...
     * World.Family.setFamily(newId, mgc.getSeniorId(), mgc.getJunior1(),
     * mgc.getJunior2(), mgc.getCurrentRep(), mgc.getTotalRep(), mgc.getId());
     * }
     * newfam.setOnline(mgc.getId(), mgc.isOnline(), mgc.getChannel());
     * }
