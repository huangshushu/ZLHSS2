        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            try (PreparedStatement ipc = con.prepareStatement("SELECT * FROM accounts WHERE macs = ?")) {
                ...
                if (rs.getRow() >= ACCOUNTS_PER_MAC) {
                    mac = false;
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系统] " + " IP：/" + sockAddr.substring(1, sockAddr.lastIndexOf(':')) + " 注册次数超过"+ACCOUNTS_PER_MAC+"次"));
                    FileoutputUtil.logToFile("logs/Data/注册超过次数.txt", "\r\n " + FileoutputUtil.NowTime() + " IP：/" + sockAddr.substring(1, sockAddr.lastIndexOf(':')) + " 注册次数超过"+ACCOUNTS_PER_MAC+"次");
                }
            }
            rs.close();
        } catch (SQLException ex) {
