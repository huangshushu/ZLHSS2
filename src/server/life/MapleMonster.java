...
public class MapleMonster extends AbstractLoadedMapleLife {
     ...
     * @param fake,                dropsDisabled, controllerHasAggro,
    ...
    private int linkoid = 0, lastNode = -1, lastNodeController = -1, highestDamageChar = 0; // Just a reference for
                                                                                            // monster EXP distribution
                                                                                            // after dead
                        ...
                        for (final AttackingMapleCharacter cattacker : mattacker.getAttackers()) {
                            if (cattacker != null && cattacker.getAttacker().getMap() == from.getMap()) { // current
                                ...
                                if (cattacker.getLastAttackTime() >= System.currentTimeMillis() - 4000) {
                                    cattacker.getAttacker().getClient().sendPacket(MobPacket.showMonsterHP(
            ...
            if (this.getHp() > 0) {
                ...
                if (this.getSponge() == null && hp > 0) {
                    switch (this.getStats().getHPDisplayType()) {
                        ...
                        case -1:
                        case 2:
                            this.getMap().broadcastMessage(MobPacket.showMonsterHP(getObjectId(),
    ...
    private void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage,
            final int numExpSharers, final byte pty, final byte classBounsExpPercent,
            final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            ...
            exp = (int) Math.min(Integer.MAX_VALUE,
                    exp * (attacker.getLevel() < 10 ? GameConstants.getExpRate_Below10(attacker.getJob())
                            : ChannelServer.getInstance(map.getChannel()).getExpRate()));
    ...
    /*
     * private void giveExpToCharacter(final MapleCharacter attacker, int exp, final
     * boolean highestDamage, final int numExpSharers, final byte pty, final byte
     * classBounsExpPercent, final byte Premium_Bonus_EXP_PERCENT, final int
     * lastskillID) {
     * 
     * 
     * //判断最高攻击
     ...
     * }
     * //检查攻击者有无 HOLY_SYMBOL Buff
     * final Integer holySymbol =
     * attacker.getBuffedValue(MapleBuffStat.HOLY_SYMBOL);
     * if (holySymbol != null) {
     * if (numExpSharers == 1) {
     * exp *= 1.0 + (holySymbol.doubleValue() / 500.0);
     * } else {
     ...
     * 
     * // exp *= attacker.getEXPMod() * (int) (attacker.getStat().expBuff / 100.0);
     * double lastexp = attacker.getStat().realExpBuff - 100.0 <= 0 ? 100 :
     * attacker.getStat().realExpBuff - 100;
     * exp *= attacker.getEXPMod() * (int) (lastexp / 100.0);
     * exp = (int) Math.min(Integer.MAX_VALUE, exp * (attacker.getLevel() < 10 ?
     * GameConstants.getExpRate_Below10(attacker.getJob()) :
     * ChannelServer.getInstance(map.getChannel()).getExpRate()));
     * //do this last just incase someone has a 2x exp card and its set to max value
     * int classBonusExp = 0;
     * if (classBounsExpPercent > 0) {
     ...
     * premiumBonusExp = (int) ((exp / 100.0) * Premium_Bonus_EXP_PERCENT);
     * }
     * int equpBonusExp = (int) ((exp / 100.0) *
     * attacker.getStat().equipmentBonusExp);
     * if (attacker.getStat().equippedFairy) {
     * equpBonusExp += (int) ((exp / 100.0) * attacker.getFairyExp());
     * }
     * 
     * attacker.gainExpMonster(exp, true, highestDamage, pty, classBonusExp,
     * equpBonusExp, premiumBonusExp);
     * 
     * }
     * attacker.mobKilled(getId(), lastskillID);
     * }
    ...
    public final void switchController(final MapleCharacter newController, final boolean immediateAggro) {
        ...
        if (getId() == 9300275 && map.getId() >= 921120100 && map.getId() < 921120500) { // shammos
            if (lastNodeController != -1 && lastNodeController != newController.getId()) { // new controller, please re
                                                                                           // update
    ...
    public final void sendSpawnData(final MapleClient client) {
        ...
        client.sendPacket(
                MobPacket.spawnMonster(this, (lastNode >= 0 ? -2 : -1), fake ? 0xfc : (lastNode >= 0 ? 12 : 0), 0));
        sendStatus(client);
        // if (reflectpack != null) {
        ...
        if (lastNode >= 0) {
            ...
            if (getId() == 9300275 && map.getId() >= 921120100 && map.getId() < 921120500) { // shammos
                if (lastNodeController != -1) { // new controller, please re update. sendSpawn only comes when you get
                                                // too far then come back anyway
                    resetShammos(client);
                } else {
    ...
    public void applyMonsterBuff(final Map<MonsterStatus, Integer> effect, final int x, final int skillId,
            long duration, final MobSkill skill, final List<Integer> reflection) {
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison,
        ...
        if (from.hasGmLevel(5)) {
            if (status.getStatus() != null) {
                from.dropMessage(6, "怪物: " + getId() + " 状态: " + status.getStatus().name() + " 是否为毒: " + poison
                        + " 持续时间: " + duration + " 技能: " + SkillFactory.getSkillName(eff.getSourceId()));
            }
        }
        ...
        if (stats.isBoss() && !status.isIgnoreBoss()) {
            ...
            if (stat == MonsterStatus.STUN) {
                return;
            } else if (stat != (MonsterStatus.SPEED) && stat != (MonsterStatus.NINJA_AMBUSH)
        ...
        if (poison && getHp() > 1) { // 中毒[POISON]
            ...
            int dam = (int) Math.min(Short.MAX_VALUE,
                    (long) (getMobMaxHp() / (70.0 - from.getSkillLevel(status.getSkill())) + 0.999));
            if (from.hasGmLevel(5)) {
            ...
            if (from.hasGmLevel(5)) {
                from.dropMessage(6, "[持续伤害] 持续伤害： " + ((getHp() > poisonDamage) ? poisonDamage : (getHp() - 1))
                        + " 持续时间：" + aniTime + " 持续掉血：" + status.getX());
            }
            ...
            status.setValue(status.getStatus(), Math.min(Short.MAX_VALUE,
        ...
        if (from.getDebugMessage()) {
            from.dropMessage(6, "开始 => 给予怪物状态: 持续时间[" + aniTime + "] 状态效果[" + status.getStatus().name() + "] 开始时间["
    ...
    public final void dispelSkill(final MobSkill skillId) {
        ...
        for (Entry<MonsterStatus, MonsterStatusEffect> effects : stati.entrySet()) {
            ...
            if (mse != null && mse.getMobSkill() != null && mse.getMobSkill().getSkillId() == skillId.getSkillId()) { // not
                                                                                                                      // checking
                                                                                                                      // for
                                                                                                                      // level.
     ...
     * if (con != null) {
     * // con.addSleep(100);
     * map.broadcastMessage(con, MobPacket.cancelMonsterStatus(this, mses),
     * getTruePosition());
     * con.getClient().sendPacket(MobPacket.cancelMonsterStatus(this, mses));
     * } else {
     * map.broadcastMessage(MobPacket.cancelMonsterStatus(this, mses),
     * getTruePosition());
     * }
     * for (MonsterStatus stat : stats) {
     * stati.remove(stat);
    ...
    public final void cancelSingleStatus(final MonsterStatusEffect stat) {
        if (stat == null || stat.getStatus() == MonsterStatus.EMPTY || stat.getStatus() == MonsterStatus.SUMMON
                || !isAlive()) {
            return;
        }
    ...
    public final void doPoison(final MonsterStatusEffect status, final WeakReference<MapleCharacter> weakChr) {
        if ((status.getStatus() == MonsterStatus.VENOMOUS_WEAPON || status.getStatus() == MonsterStatus.POISON
                || status.getStatus() == MonsterStatus.NEUTRALISE) && poisons.size() <= 0) {
            return;
        }
        if (status.getStatus() != MonsterStatus.VENOMOUS_WEAPON && status.getStatus() != MonsterStatus.POISON
                && status.getStatus() == MonsterStatus.NEUTRALISE && !stati.containsKey(status.getStatus())) {
            return;
        }
        if (weakChr == null) {
            return;
    ...
    private class PartyAttackerEntry implements AttackerEntry {
        ...
        public final void killedMob(final MapleMap map, final int baseExp, final boolean mostDamage,
            ...
            for (final Entry<MapleCharacter, OnePartyAttacker> attacker : resolveAttackers().entrySet()) {
                 ...
                 * expWeight = (expReceiver == attacker.getKey() ? 2.0 : 0.6); //hopefully this
                 * is correct o.o -/+0.4
                 ...
                 * expMap.put(expReceiver, new ExpMap(iexp, (byte) (expApplicable.size() +
                 * added_partyinc), Class_Bonus_EXP, Premium_Bonus_EXP));
                 * }
                 */
                for (final MapleCharacter expReceiver : expApplicable) {
                    iexp = expMap.get(expReceiver) == null ? 0 : expMap.get(expReceiver).exp;
        ...
        /*
         * @Override
         * public final void killedMob(final MapleMap map, final int baseExp, final
         * boolean mostDamage, final int lastSkill) {
         * MapleCharacter pchr, highest = null;
         * long iDamage, highestDamage = 0;
         * int iexp;
         * MapleParty party;
         ...
         * iexp += (int) Math.round(((attacker.getKey().getId() == expReceiver.getId() ?
         * 0.6 : 0.0) + levelMod) * innerBaseExp);
         * expMap.put(expReceiver, new ExpMap(iexp, (byte) (expApplicable.size() +
         * added_partyinc), Class_Bonus_EXP, Premium_Bonus_EXP));
         * }
         * }
         ...
         * for (final Entry<MapleCharacter, ExpMap> expReceiver : expMap.entrySet()) {
         * expmap = expReceiver.getValue();
         * giveExpToCharacter(expReceiver.getKey(), expmap.exp, mostDamage ?
         * expReceiver.getKey() == highest : false, expMap.size(), expmap.ptysize,
         * expmap.Class_Bonus_EXP, expmap.Premium_Bonus_EXP, lastSkill);
         * }
         * }
         */
            ...
            for (MonsterDropEntry d : dropEntry) {
                if (d.itemId > 0 && d.questid == 0 && d.itemId / 10000 != 238
                        && Randomizer.nextInt(999999) < (int) (10 * d.chance * chServerrate * chr.getDropMod()
                                * chr.getDropm() * ((chr.getVipExpRate() / 100D) + 1.0D)
