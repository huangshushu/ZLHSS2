...
public class AramiaFireWorks {
    ...
    public final void giveKegs(final MapleCharacter c, final int kegs) {
        ...
        switch (this.kegs) {
            case 1000:
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "<频道 " + c.getClient().getChannel() + "> " + "不夜城新年活动进度目前是5000/" + this.kegs + "！！"));
                break;
            case 2000:
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "<频道 " + c.getClient().getChannel() + "> " + "不夜城新年活动进度目前是5000/" + this.kegs + "！！"));
                break;
    ...
    private void broadcastServer(final MapleCharacter c, final int itemid) {
        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, itemid, "<频道 " + c.getClient().getChannel() + "> " + "不夜城新年活动即将开始举办怪物大游行！！"));
    }
    ...
    private final void startDec(final MapleMap map) {
        map.startMapEffect("圣诞树正在绽放！", 5120000);
        for (int i = 0; i < 3; i++) {
            EventTimer.getInstance().schedule(new Runnable() {
