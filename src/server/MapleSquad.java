...
public class MapleSquad {
            ...
            if (theirCh > 0) {
                ...
                if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
                    ...
                    if (ChannelServer.getInstance(ch).addMapleSquad(squad, type.name())) {
                        ...
                        squad.clear();
                        type.queuedPlayers.get(ch).add(new Pair<>(nextPlayerId, "跳过"));
                    }
                    break;
                } else {
                    if (lead != null) {
                    ...
                    getBeginMap().broadcastMessage(MaplePacketCreator.serverNotice(6, nextPlayerId + "远征队已经结束了，由于有成员没有在地图内"));
                    type.queuedPlayers.get(ch).add(new Pair<>(nextPlayerId, "不在地图内"));
                }
            } else {
                getBeginMap().broadcastMessage(MaplePacketCreator.serverNotice(6, nextPlayerId + "'远征队已经结束了，由于有成员没有在线上"));
                type.queuedPlayers.get(ch).add(new Pair<>(nextPlayerId, "没有上线"));
            }
        }
    }
        ...
        if (join) {
            if (!containsMember(member) && !getAllNextPlayer().contains(member.getName())) {
                if (members.size() <= 30) {
                    ...
                    getLeader().dropMessage(6, member.getName() + " (" + job + ") 加入了远征队!");
                    return 1;
                }
                return 2;
            }
    ...
    public void acceptMember(int pos) {
        ...
        if (toadd != null && getChar(toadd) != null) {
            ...
            getChar(toadd).dropMessage(5, getLeaderName() + " 允许你重新回来远征队");
        }
    }
    ...
    public void banMember(int pos) {
        ...
        if (toban != null && getChar(toban) != null) {
            ...
            getChar(toban).dropMessage(5, getLeaderName() + " 从远征队中删除您.");
        }
    }
    ...
    public String getSquadMemberString(byte type) {
        switch (type) {
            case 0: {
                StringBuilder sb = new StringBuilder("目前远征队总人数 : ");
